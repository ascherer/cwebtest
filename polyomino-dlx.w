...
`\.{[12]7suf}' specifies two columns named `\.{17suf}' and `\.{27suf}'.
Such columns will be {\it secondary}.
...
All suffixes associated with a cell will be appended to the columns
generated by that cell. For example, a piece that has typical cells
`\.{00}, \.{01}, \.{00!}, \.{01!}' will generate rows for every
pair of adjacent cells in the box: When \.{34} and \.{44} are present,
there will be a row `\.{34} \.{44} \.{34!} \.{44!}'.
If \.{01!} hadn't been specified, there
would have been {\it two\/} rows, `\.{34} \.{44} \.{34!}' and
`\.{44} \.{34} \.{44!}'.
...
\.{00} isn't in the box. This condition is not checked by the program.
...
@<Subroutines@>;
main () {
  register int i,j,k,p,q,r,t,x,y,dx,dy,xy0;
...
typedef struct {
  ...
  char suf[8]; /* suffix data for this node (often empty) */
...
@<Sub...@>=
void insert(int x,int y,char s[]) {
  register int p,q,r,xy;
  xy=(x<<8)+y;
  for (q=0,p=newbox.list;p;q=p,p=elt[p].link) {
    if (elt[p].xy==xy) {
      r=strcmp(elt[p].suf,s);
      if (r==0) return; /* nothing to be done */
      if (r>0) break; /* we've found the insertion point */
    }@+else if (elt[p].xy>xy) break; /* we've found the insertion point */
  }
  r=getavail();
  strcpy(elt[r].suf,s);
  elt[r].xy=xy, elt[r].link=p;
  if (q) elt[q].link=r;
  else newbox.list=r;
  newbox.size++;
...
them in a two-dimensional array called |occupied|.
...
@<Scan an $xy$ spec@>=
{
  ...
  @<Put the optional suffix into |sfx|@>;
...
@ @<Set up the |occupied| table@>=
for (p=givenbox.list;p;p=elt[p].link) if (!elt[p].suf[0]) {
  x=elt[p].xy>>8, y=elt[p].xy&0xff;
  occupied[x][y]=1;
}
...
char sfx[8]; /* the current suffix */
char occupied[64][64]; /* does the box occupy a given cell? */
box givenbox;
int sfxpresent;
...
int equality(int b) { /* return 1 if |base[b]| matches |newbox| */
  ...
  for (p=base[b].list,q=newbox.list; p; p=elt[p].link,q=elt[q].link)
    if (elt[p].xy!=elt[q].xy || strcmp(elt[p].suf,elt[q].suf)) return 0;
  return 1;
}
...
while (j<k) {
  ...
  @<Set |newbox| to |base[j]| transformed by |xy| cycling@>;
  for (i=basecount;i<k;i++)
    if (equality(i)) break;
  if (i<k) putavail(newbox.list); /* already known */
  else base[k++]=newbox; /* we've found a new one */
...
if (sfxpresent) {
  printf(" |");
  for (sfxpresent=0,p=givenbox.list;p;p=elt[p].link) if (elt[p].suf[0]) {
...
for (j=0;j<basecount;j++) {
  for (dx=givenbox.xmin;dx<=givenbox.xmax-base[j].xmax;dx++)
   for (dy=givenbox.ymin;dy<=givenbox.ymax-base[j].ymax;dy++) {
      for (p=base[j].list;p;p=elt[p].link) if (!elt[p].suf[0]) {
        x=elt[p].xy>>8, y=elt[p].xy&0xff;
        if (!occupied[x+dx][y+dy]) break;
      }
      if (!p) { /* they're all in the box */
        ...
        for (p=base[j].list;p;p=elt[p].link) {
          x=elt[p].xy>>8, y=elt[p].xy&0xff;
          printf(" %c%c%s",
                  encode(x+dx),encode(y+dy),elt[p].suf);
        }
        printf("\n");
      }
    }
