...
\N[4 celtic-paths.w]{1}{1}Intro. This quick-and-dirty program prepares \TeX\
files for use with
...
\M[89 celtic-paths.w]{2}Whenever a tile contains a segment through some
boundary point,
...
\M[101 celtic-paths.w]{3}OK, let's get going. This program ought to be fun,
once we get through
...
\M[128 celtic-paths.w]{4}(Begin tedium.)
...
\M[197 celtic-paths.w]{5}(More tedium. I do try to check carefully for errors,
because the task
...
\M[237 celtic-paths.w]{6}\B\X6:Parse the entry for row \PB{\|m} and column \PB{%
\|j}\X${}\E{}$\6
...
\M[262 celtic-paths.w]{7}\B\D$\\{eqbit}(\|k,\\{kk})$ \5
...
\M[289 celtic-paths.w]{8}OK, now the fun begins: We've got decent input, and we
want to figure out
how to typeset it.
...
\M[317 celtic-paths.w]{9}The inside/outside coloring is trickier, because
connectivity to the
...
\M[338 celtic-paths.w]{10}The neighbors of a subregion within a tile are either
in an adjacent tile
or in the same tile.
...
\M[353 celtic-paths.w]{11}A tile that contains two nonintersecting segments
consists of a
...
\M[380 celtic-paths.w]{12}\B\X12:Check neighbors in three-region tiles\X${}%
\E{}$\6
...
\M[389 celtic-paths.w]{13}And at last, when every subregion has been colored in
both of the
...
\M[417 celtic-paths.w]{14}\B\X14:Publish the preamble\X${}\E{}$\6
...
\M[427 celtic-paths.w]{15}\B\X15:Publish the postamble\X${}\E{}$\6
\\{printf}(\.{"\}\\\\endgroup\\n"});\par
\U13.\fi

\M[430 celtic-paths.w]{16}\B\X16:Publish row \PB{\|i}\X${}\E{}$\6
${}\{{}$\1\6
...
\M[437 celtic-paths.w]{17}\B\X17:Typeset tile $(i,j)$\X${}\E{}$\6
${}\{{}$\1\6
...
\M[452 celtic-paths.w]{18}\B\X18:Handle a blank tile\X${}\E{}$\6
${}\{{}$\1\6
\&{if} (\\{inout}[\|i][\|j][\T{0}])\5
...
\M[459 celtic-paths.w]{19}\B\X19:Handle a two-region tile\X${}\E{}$\6
${}\{{}$\1\6
...
\M[466 celtic-paths.w]{20}The fonts treat \.{1537} as a special case in which
all sixteen
...
\M[479 celtic-paths.w]{21}In the most complex case, we walk clockwise around
