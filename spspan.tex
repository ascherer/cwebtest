...
\N[5 spspan.w]{1}{1}Introduction. This program generates all spanning trees of
a given
...
\N[57 spspan.w]{1}{2}Parsing and preparation. We begin by converting the Polish
notation
into a binary tree.
...
\M[83 spspan.w]{4}Mems are not counted in this phase of the operation, because
the
...
\M[90 spspan.w]{5}\B\X5:Create a new branch\X${}\E{}$\6
${}\{{}$\1\6
\&{if} ${}(\|k<\T{2}){}$\1\5
...
\M[98 spspan.w]{6}Now we convert the binary tree to the desired working tree,
whose branch
...
\M[110 spspan.w]{7}The first half of \PB{\\{nodelist}} contains up to \PB{%
\\{maxn}} leaves;
...
\M[119 spspan.w]{8}A recursive subroutine called \PB{\\{build}} will govern the
...
\M[130 spspan.w]{9}When we \PB{\\{build}} a leaf node, we simply allocate it.
When we
...
\M[155 spspan.w]{10}\B\X10:Incorporate right child into node \PB{\|p}\X${}\E{}$%
\6
$\|r\K\|p\MG\\{lchild},\39\|p\MG\\{lchild}\K\|l,\39\|l\MG\\{rsib}\K\|r,\39\|p%
\MG\\{rchild}\MG\\{rsib}\K\|l{}$;\par
\U9.\fi

\M[158 spspan.w]{11}\B\X11:Incorporate left child into node \PB{\|p}\X${}\E{}$\6
\&{if} ${}(\|r\E\|p){}$\1\5
...
\M[162 spspan.w]{12}\B\X12:Incorporate both children into node \PB{\|p}\X${}%
\E{}$\6
$\\{rc}\MG\\{rsib}\K\|p\MG\\{lchild},\39\|p\MG\\{lchild}\K\\{lc},\39\|p\MG%
\\{rchild}\MG\\{rsib}\K\\{lc}{}$;\par
\U11.\fi

\M[165 spspan.w]{13}OK, the tree has been set up; our next goal is to decorate
it.
...
\M[233 spspan.w]{14}To start things off, we might as well designate each node's
leftmost child.
...
\M[244 spspan.w]{15}A few amendments to the data structure will be desirable
later, but we're
...
\N[263 spspan.w]{1}{16}Diagnostic routines. Several simple subroutines are used
to
...
\M[284 spspan.w]{17}\B\X9:Subroutines\X${}\mathrel+\E{}$\6
...
\M[306 spspan.w]{18}\B\X9:Subroutines\X${}\mathrel+\E{}$\6
...
\M[319 spspan.w]{19}\B\X9:Subroutines\X${}\mathrel+\E{}$\6
...
\M[331 spspan.w]{20}\B\X20:Print the first tree\X${}\E{}$\6
${}\{{}$\1\6
\&{if} (\\{extraverbose})\1\5
...
\N[339 spspan.w]{1}{21}Overview of the algorithm. A branch node \PB{\|p} will
be called {\it easy\/}
...
\M[358 spspan.w]{22}Cartesian products of configurations are easily generated
in Gray-code
...
\M[380 spspan.w]{23}These considerations lead us to the following algorithm to
generate
...
\M[400 spspan.w]{24}For example, let's apply the algorithm to the
...
\M[446 spspan.w]{25}So-called ``focus pointers'' can be used to implement steps
(1) and (3)
...
\M[467 spspan.w]{26}We can easily incorporate the new fields into our
initialization
...
\M[475 spspan.w]{27}\B\X27:Further initialization of a branch node\X${}\E{}$\6
$\|p\MG\\{leaf}\K\|p\MG\\{des}\MG\\{leaf},\39\|p\MG\\{parent}\K\\{par};{}$\6
${}\|o,\39\|p\MG\\{focus}\K\|p{}$;\par
\U15.\fi

\M[479 spspan.w]{28}\B\X28:Print additional fields of a branch node\X${}\E{}$\6
$\\{printf}(\.{"\ leaf=\%c"},\39\\{leafname}(\|p\MG\\{leaf}));{}$\6
\&{if} ${}(\|p\MG\\{focus}\I\|p){}$\1\5
${}\\{printf}(\.{"\ focus=\%c"},\39\\{branchname}(\|p\MG\\{focus})){}$;\2\par
\U17.\fi

\N[483 spspan.w]{1}{29}Doing it. Let's go ahead now and implement the algorithm
just sketched.
...
\M[496 spspan.w]{30}All uneasy nodes to the right of \PB{\|p} are now active,
and \PB{\|l} is the
former \PB{$\|p\MG\\{des}$}.
...
\M[506 spspan.w]{31}If the user has asked for \PB{\\{verbose}} output, we print
only the
...
\N[530 spspan.w]{1}{32}A loopless version. The algorithm implemented here
contains four
