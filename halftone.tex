...
\N[2 halftone.w]{1}{1}Introduction. This program prepares data for the examples
in my
...
\M[51 halftone.w]{2}Optional command-line arguments allow the user to multiply
the diffusion
...
\M[62 halftone.w]{3}Macintosh conventions indicate the end of a line by the
...
\M[88 halftone.w]{4}\B\X4:Global variables\X${}\E{}$\6
\&{int} \\{llx}${},{}$ \\{lly}${},{}$ \\{urx}${},{}$ \\{ury};\C{ bounding box
parameters }\par
\A8.
\U1.\fi

\M[91 halftone.w]{5}After we've seen the bounding box, we look for %
\.{beginimage\char`\\r};
...
\M[112 halftone.w]{6}Photoshop follows the conventions of photographers who
consider
...
\M[125 halftone.w]{7}In the rotated case, we transpose and partially shift the
input so that
...
\N[178 halftone.w]{1}{8}Diffusing the error. We convert the darkness values to
...
\M[211 halftone.w]{9}In the main loop, we will want to find the best
approximation to
...
\M[229 halftone.w]{10}The rectangular case is simplest, so we consider it
first. Our strategy
...
\M[252 halftone.w]{11}The rotated case is essentially the same, but the
unprocessed
...
\M[276 halftone.w]{12}Finally we are ready to put everything together.
