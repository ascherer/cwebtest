...
\N[8 polynum.w]{1}{1}Introduction. The purpose of this program is to enumerate
fixed polyominoes
...
\M[79 polynum.w]{2}A bit of theory may help the reader internalize these rules:
It turns out
...
\M[101 polynum.w]{3}Any polyomino with the configuration $(*)$ in the midst of
row 10 must have
...
\M[119 polynum.w]{4}This program doesn't actually do the whole job of
enumeration;
...
\M[140 polynum.w]{5}\B\X5:Subroutines\X${}\E{}$\6
...
\M[147 polynum.w]{6}The user specifies the maximum size of polyominoes to be
counted, $n$,
...
\M[180 polynum.w]{7}\B\X7:Global variables\X${}\E{}$\6
...
\N[188 polynum.w]{1}{8}Output. Let's get the basics of output out of the way
first, so that
...
\M[218 polynum.w]{9}\B\X5:Subroutines\X${}\mathrel+\E{}$\6
...
\M[230 polynum.w]{10}\B\X5:Subroutines\X${}\mathrel+\E{}$\6
...
\M[240 polynum.w]{11}\B\X5:Subroutines\X${}\mathrel+\E{}$\6
...
\M[257 polynum.w]{12}A four-byte instruction has the binary form
...
\M[298 polynum.w]{13}\B\D$\\{end\_of\_buffer}$ \5
...
\M[314 polynum.w]{14}\B\X5:Subroutines\X${}\mathrel+\E{}$\6
...
\M[333 polynum.w]{15}The first six bytes of the instruction file are, however,
special.
...
\M[356 polynum.w]{16}Here's what we'll do when it's all over.
...
\M[362 polynum.w]{17}Most of the output is generated by the \PB{\\{basic%
\_inst}} routine.
...
\M[385 polynum.w]{18}\B\X7:Global variables\X${}\mathrel+\E{}$\6
...
\N[390 polynum.w]{1}{19}Connectivity. The hardest task that confronts us is to
figure out
...
\M[427 polynum.w]{20}The situation gets more interesting when components are
nested. Suppose, for
...
\M[445 polynum.w]{21}Now look at the configuration \.{-00(010-010)00()00()00-},
which is
...
\M[471 polynum.w]{22}A recursive strategy can be used to solve the minimum
...
\M[520 polynum.w]{23}If $\alpha$ and $\beta$ are subcomponents that are already
connected to each
...
\M[541 polynum.w]{24}Our discussion so far has assumed for simplicity that
we're at the end
...
\M[553 polynum.w]{25}After calculating the minimum cost of connection, we might
also need to
...
\M[576 polynum.w]{26}OK, we've got a reasonably clean theory; now it's time to
put it into
...
\M[588 polynum.w]{27}\B\X7:Global variables\X${}\mathrel+\E{}$\6
...
\M[592 polynum.w]{28}Then in the cost matrices and in our answer describing the
minimum
...
\M[609 polynum.w]{29}We will need to distinguish between the cost matrices
...
\M[618 polynum.w]{30}A few easy subroutines do the basic operations on cost
...
\M[632 polynum.w]{31}\B\X5:Subroutines\X${}\mathrel+\E{}$\6
...
\M[645 polynum.w]{32}\B\X5:Subroutines\X${}\mathrel+\E{}$\6
...
\M[654 polynum.w]{33}The algorithm we use is inherently recursive, but we
implement it
...
\M[674 polynum.w]{34}The given configuration string will appear in \PB{\|c[%
\T{1}]} through \PB{\|c[\|w]};
...
\M[681 polynum.w]{35}This program operates in two phases that are almost
identical:
...
\M[724 polynum.w]{36}In practice \PB{\\{row\_end}} will not be zero. But I
...
\M[738 polynum.w]{37}\B\X37:Scan for zeros in Phase 0\X${}\E{}$\6
\&{if} ${}(\|k\E\\{row\_end}){}$\5
${}\{{}$\1\6
...
\M[755 polynum.w]{38}\B\X38:Scan a nonzero token cluster in Phase 0\X${}\E{}$\6
$\\{cm}\K\\{base\_cost0};{}$\6
...
\M[785 polynum.w]{39}\B\X7:Global variables\X${}\mathrel+\E{}$\6
...
\M[795 polynum.w]{40}\B\X40:Compress $\.-^*\.)$ following \.( into a single
...
\M[805 polynum.w]{41}\B\X41:Compress $\.-^*\.)$ following \.- into a single
token in Phase 0\X${}\E{}$\6
${}\{{}$\1\6
\&{do}\5
${}\{{}$\1\6
...
\M[815 polynum.w]{42}\B\X42:Append \PB{\\{cm}} to the current partial component%
\X${}\E{}$\6
\&{if} (\|s)\1\5
...
\M[819 polynum.w]{43}\B\X43:Append \PB{\\{cm}} to \PB{$\\{stk}[\|s].\\{open%
\_cost}$}\X${}\E{}$\6
${}\{{}$\1\6
...
\M[826 polynum.w]{44}\B\X44:Combine the top two items on the stack\X${}\E{}$\6
$\|s\MM;{}$\6
\&{if} (\|s)\1\5
...
\M[832 polynum.w]{45}\B\X45:Append \PB{$\\{stk}[\|s+\T{1}].\\{open\_cost}$} to %
\PB{$\\{stk}[\|s].\\{open\_cost}$}\X${}\E{}$\6
${}\{{}$\1\6
...
\M[840 polynum.w]{46}\B\X46:Finish processing \PB{\\{lft}}\X${}\E{}$\6
$\\{stk}[\PP\|s].\\{gap\_typ}\K\\{typ};{}$\6
${}\\{stk}[\|s].\\{gap}\K\|g;{}$\6
${}\\{stk}[\|s].\\{closed\_cost}\K\\{stk}[\|s].\\{open\_cost}\K\\{cm}{}$;\par
\Us38\ET49.\fi

\M[845 polynum.w]{47}\B\X47:Finish processing \PB{\\{mid}}\X${}\E{}$\6
\&{if} ${}(\R\|s){}$\5
${}\{{}$\1\6
\&{if} ${}(\\{stk}[\T{0}].\\{closed\_cost}.\|c[\T{1}][\T{1}]){}$\1\5
\\{panic}(\.{"Unmatched\ -"});\2\6
...
\M[854 polynum.w]{48}\B\X48:Scan for zeros in Phase 1\X${}\E{}$\6
\&{if} (\|c[\|k])\1\5
\\{panic}(\.{"Syntax\ error,\ 0\ exp}\)\.{ected"});\2\6
...
\M[861 polynum.w]{49}\B\X49:Scan a nonzero token cluster in Phase 1\X${}\E{}$\6
$\\{cm}\K\\{base\_cost1};{}$\6
${}\|k\PP;$ \&{switch} ${}(\|c[\|k-\T{1}])$ $\{$ \6
\4\&{case} \\{lft}:\5
\&{if} ${}(\R\\{mid\_or\_rt}(\|c[\|k])){}$\1\5
...
\M[890 polynum.w]{50}\B\X50:Compress $\.-^*\.)$ following \.( into a single
...
\M[902 polynum.w]{51}\B\X51:Compress $\.-^*\.)$ following \.- into a single
token in Phase 1\X${}\E{}$\6
${}\{{}$\1\6
\4\\{scan\_tokens1b}:\5
\&{do}\5
...
\M[914 polynum.w]{52}The cost of reaching the right edge is \PB{\|g} if \PB{%
\|g} has the form
...
\N[929 polynum.w]{1}{53}Data structures. If we want to count $n$-ominoes
...
\M[985 polynum.w]{54}Eight bytes suffice to encode any configuration $c_0\ldots
c_{w-1}$ in a
...
\M[1001 polynum.w]{55}The \PB{\\{pack}} subroutine produces a \PB{\&{cstring}}
from the codes in array \PB{\|c}.
...
\M[1026 polynum.w]{56}That which can be packed can be unpacked. This routine
puts the
...
\M[1062 polynum.w]{57}\B\X7:Global variables\X${}\mathrel+\E{}$\6
\&{code} \\{sc}[\T{64}];\C{ codes of the current source configuration }\par
\fi

\M[1065 polynum.w]{58}\B\X15:Initialize\X${}\mathrel+\E{}$\6
$\|c[\T{0}]\K\\{sc}[\T{0}]\K\|c[\|w+\T{1}]\K\\{sc}[\|w+\T{1}]\K\\{eol}{}$;\par
\fi

\M[1068 polynum.w]{59}\B\X5:Subroutines\X${}\mathrel+\E{}$\6
...
\M[1079 polynum.w]{60}A configuration can be in three states: Normally it is %
\PB{\\{active}},
...
\M[1089 polynum.w]{61}Here then are the 20 precious bytes that represent a
configuration.
...
\M[1103 polynum.w]{62}\B\X15:Initialize\X${}\mathrel+\E{}$\6
...
\M[1108 polynum.w]{63}The main high-level routine, called \PB{\\{update}}, is
used to add terms
...
\M[1126 polynum.w]{64}``Universal hashing'' ({\sl TAOCP\/} exercise 6.4--72) is
used to get a
...
\M[1141 polynum.w]{65}\B\X7:Global variables\X${}\mathrel+\E{}$\6
...
\M[1145 polynum.w]{66}The random number generator used here doesn't have to be
of sensational
...
\M[1155 polynum.w]{67}\B\X67:Local variables\X${}\E{}$\6
...
\M[1159 polynum.w]{68}On odd-numbered passes, \PB{\\{src}} runs down towards %
\PB{\\{conf}}, while \PB{\\{trg}}
...
\M[1172 polynum.w]{69}When a new configuration is created, we allocate space
for its
...
\M[1202 polynum.w]{70}\B\X70:Allocate \PB{$\|p\MG\\{addr}$} (upward) and check
...
\M[1218 polynum.w]{71}\B\X7:Global variables\X${}\mathrel+\E{}$\6
...
\M[1234 polynum.w]{72}\B\X72:Print statistics about this run\X${}\E{}$\6
\\{printf}(\.{"Altogether\ "});\6
\&{if} (\\{hconfigs})\1\5
...
\M[1245 polynum.w]{73}\B\X5:Subroutines\X${}\mathrel+\E{}$\6
...
\M[1260 polynum.w]{74}\B\X74:Allocate \PB{$\|p\MG\\{addr}$} (downward) and
...
\M[1276 polynum.w]{75}The \PB{\\{move\_down}} and \PB{\\{move\_up}} subroutines
are invoked when an
...
\M[1305 polynum.w]{76}\B\X5:Subroutines\X${}\mathrel+\E{}$\6
...
\N[1325 polynum.w]{1}{77}The main loop. Now that we have some infrastructure in
place,
...
\M[1347 polynum.w]{78}Once again it seems best to write two nearly identical
pieces of
...
\M[1360 polynum.w]{79}\B\X79:Get ready for a downward pass, or \PB{\&{break}}
when done\X${}\E{}$\6
\&{if} ${}(\\{row\_end}<\|w){}$\5
${}\{{}$\1\6
${}\\{row\_end}\PP;{}$\6
...
\M[1381 polynum.w]{80}\B\X80:Pass downward over all configurations created on
...
\M[1399 polynum.w]{81}Timely progress reports let the user know that we are
...
\M[1411 polynum.w]{82}Counter positions 1 through $n$ in the slave memory are
reserved for
...
\M[1435 polynum.w]{83}\B\X83:Pass upward over all configurations created on the
previous pass\X${}\E{}$\6
...
\N[1453 polynum.w]{1}{84}Nitty-gritty. The basic logic of a so-called
...
\M[1555 polynum.w]{85}\B\X7:Global variables\X${}\mathrel+\E{}$\6
\&{int} \\{pair};\C{ the two codes surrounding the \qcaret/ in \PB{\\{sc}} }\6
\&{int} \\{viable};\C{ might the target configuration lead to a relevant
polyomino? }\par
\fi

\M[1559 polynum.w]{86}In this step, we have just zeroed out a left parenthesis.
...
\M[1586 polynum.w]{87}Contrariwise, an erased \.) is like an erased \.( but
vice versa.
...
\M[1603 polynum.w]{88}\B\X88:Cautiously delete a \PB{\\{mid}} that may be
leftmost\X${}\E{}$\6
...
\M[1626 polynum.w]{89}\B\X89:Cautiously delete a \PB{\\{mid}} that may be
rightmost\X${}\E{}$\6
\&{for} ${}(\|k\K\\{row\_end}+\T{1};{}$ ${}\|c[\|k]\E\\{zero};{}$ ${}\|k\PP){}$%
\1\5
;\2\6
...
\M[1649 polynum.w]{90}A different kind of excitement awaits us when we consider
...
\M[1759 polynum.w]{91}\B\X91:Merge with the mate of the former \PB{\\{lft}}%
\X${}\E{}$\6
\&{for} ${}(\|k\K\\{row\_end}+\T{1},\39\|j\K\T{0};{}$  ; ${}\|k\PP){}$\5
${}\{{}$\1\6
\&{switch} (\|c[\|k])\5
...
\M[1771 polynum.w]{92}\B\X92:Merge with the mate of the former \PB{\\{rt}}\X${}%
\E{}$\6
...
\M[1783 polynum.w]{93}\B\X93:Downgrade the next component if it is open\X${}%
\E{}$\6
\&{for} ${}(\|k\K\T{2};{}$  ; ${}\|k\PP){}$\5
${}\{{}$\1\6
\&{switch} (\|c[\|k])\5
...
\M[1794 polynum.w]{94}\B\X94:Cautiously introduce a new \PB{\\{one}}\X${}\E{}$\6
$\|c[\\{row\_end}]\K\\{one};{}$\6
...
\M[1815 polynum.w]{95}\B\X95:Make special corrections at the right edge\X${}%
\E{}$\6
\&{switch} (\|c[\\{row\_end}])\5
${}\{{}$\1\6
\4\&{case} \\{rt}:\5
...
\M[1823 polynum.w]{96}\B\X96:Downgrade the previous component if it is open%
\X${}\E{}$\6
...
\N[1834 polynum.w]{1}{97}Nittier-and-grittier. The last nontrivial hurdle
facing us
...
\M[1864 polynum.w]{98}\B\X7:Global variables\X${}\mathrel+\E{}$\6
\&{cstring} \\{target};\C{ the packed name of the current target configuration
}\6
\&{int} \\{hash};\C{ its hash address }\par
\fi

\M[1868 polynum.w]{99}\B\X67:Local variables\X${}\mathrel+\E{}$\6
\&{register} \&{config} ${}{*}\|p{}$;\C{ current target of interest }\par
\fi

\M[1871 polynum.w]{100}At the end of a row we change the configuration to its
left-right
...
\M[1891 polynum.w]{101}We take care to move \PB{\|p} to the top of its hash
list, when present,
...
\M[1908 polynum.w]{102}If the target is viable, \PB{\|p} will be set to a fresh
node with
...
\M[1929 polynum.w]{103}\B\X103:Process target configuration \PB{\|c} (upward)%
\X${}\E{}$\6
${}\{{}$\1\6
\&{if} ${}(\\{row\_end}\E\|w){}$\1\5
\X100:Canonize the configuration\X;\2\6
...
\M[1950 polynum.w]{104}\B\X104:Get an upward slot for \PB{\\{target}} if it is
really viable\X${}\E{}$\6
${}\{{}$\1\6
${}\|j\K\\{connectivity}(\\{row\_end}+\T{1});{}$\6
\&{if} ${}(\|r\G\|w){}$\1\5
...
\N[1967 polynum.w]{1}{105}Checkpointing. One of the goals of this program is to
establish
...
\M[2001 polynum.w]{106}A special \PB{\\{sync}} instruction with parameter 255
tells {\mc POLYSLAVE}
...
\M[2010 polynum.w]{107}Since we're at the beginning of a downward pass, the
user will be
...
\M[2025 polynum.w]{108}\B\X7:Global variables\X${}\mathrel+\E{}$\6
\&{int} \\{dump\_data}[\T{5}];\C{ parameters needed to restart }\par
\fi

\N[2028 polynum.w]{1}{109}Computational experience. With a suitable change file
...
\M[2084 polynum.w]{110}\B\X7:Global variables\X${}\mathrel+\E{}$\6
...
\M[2089 polynum.w]{111}The greatest difficulty for $n=47$ occurred when $w=20$;
indeed,
...
\M[2135 polynum.w]{112}I was glad to see that the allocation system used here
for variable-length
...
\M[2143 polynum.w]{113}Joke: George P\'olya was a polymath who worked on
polyominoes.

\fi

\N[2146 polynum.w]{1}{114}Index.
\fi
