...
\N{1}{1}Introduction. This program does calculations with skew ternary trees,
...
\M{2}Fact: {\sl Every family of $2n+2$ cyclically equivalent ternary trees
...
\M{3}The four skew ternary trees of a cyclic family turn out to have
...
\M{4}One of the first goals of this program will be to compute the
...
\M{5}OK, we now know the definition of skew ternary trees, and it's time
...
\M{6}We don't deal with the empty tree; there must be at least one node.
...
\N{1}{7}Parsing. First things first: We gotta get the tree into memory
...
\M{9}The initial setup is straightforward, although a bit tedious.
...
\M{10}We need to locate the root, which should be the unique input node
...
\M{11}The \PB{\\{setmate}} subroutine allocates two new buds. Its parameter
names
...
\M{12}The main work of filling buds and setting ranks is done by a
straightforward
...
\M{13}We've prepared a skewed ternary tree by filling in all the missing
...
\M{15}\B\X8:Global variables\X${}\mathrel+\E{}$\6
...
\M{16}\B\X16:Find and print the three conjugates of \PB{\|T}\X${}\E{}$\6
...
\M{17}The state chart is created from a recursive routine \PB{\\{createsteps}},
...
\M{19}\B\X11:Subroutines\X${}\mathrel+\E{}$\6
...
\M{20}\B\X20:Create the state chart\X${}\E{}$\6
...
\M{21}Conversely, given the state chart, there's a simple recursive routine
...
\M{22}The algorithm here is very cute, so I let the reader have the
...
\M{23}\B\X23:Print the tree with all buds shown\X${}\E{}$\6
\\{print\_tree}(\\{root});\6
\\{printf}(\.{"\\n"});\par
\U16.\fi

\M{24}\B\X11:Subroutines\X${}\mathrel+\E{}$\6
...
\N{1}{25}The quad-edge data structure for planar maps.
...
\M{26}Notice that each of the edges $\{a,b,c,d,e,f\}$ in $(*)$ appears as
...
\M{27}Suppose there are $m$ edges. Diagram $(**)$ can also be regarded as a
...
\M{28}Thus the quad-edge data structure for a planar graph with $m$ edges
...
\M{29}Armed with all this information, we're ready at last to construct
...
\M{30}So let's write that code. Each pip is conveniently represented
...
\M{31}We'll permute only the pips for a special {\it root edge\/}
...
\M{32}The \PB{\\{splice}} subroutine is given the addresses of two vertex pips
...
\M{33}Here's a cute subroutine that displays all relevant information about
...
\M{34}The idea is to find a cycle leader (the least \PB{\|p} whose cycle hasn't
...
\M{35}Exactly the same idea works for odd-numbered pips, of course.
...
\N{1}{36}The building blocks of planar graphs.
...
\M{37}The simplest \RNBPM/ consists of just the root edge. Otherwise
...
\M{38}Here, for handy reference, are the smallest \RNBPM/s and their
...
\N{1}{39}Planar maps, conform\`emente \`a Jacquard et Schaeffer.
...
\M{40}To complete the construction, we need to explain how to represent
...
\M{41}Our principal goal is to take a given skew ternary tree and to
...
\M{42} The \PB{\\{rnbpm\_js}} routine constructs the Jacquard--Schaeffer
...
\M{43}\B\X43:Determine the number \PB{\|m} of initial rank~0 nodes, and stack
them\X${}\E{}$\6
...
\M{44}At this point we're poised to look at the steps of $T^*$, where
...
\M{45}The \RNBPM/ for an empty tree is simply the unadorned root edge \PB{\|r}.
...
\M{46}There's a better way to do this step, because we can identify the pip %
\PB{\|p}
...
\M{47}\B\X47:Splice the new \RNBPM/ to the previous fragment\X${}\E{}$\6
$\\{splice}(\\{irot}(\|p),\39\\{apip});{}$\6
${}\\{apip}\K\\{pip}(\|t,\39\T{2}){}$;\par
\U42.\fi

\M{48}\B\X48:Splice everything into a cycle\X${}\E{}$\6
$\\{splice}(\\{pip}(\|r,\39\T{0}),\39\\{apip}){}$;\par
\U42.\fi

\M{49}Okay, \PB{\\{rnbpm\_js}} is finished.
...
\M{50}Unfortunately, I must report serious disappointment with this
correspondence
...
\N{1}{51}Planar maps, conformemente Del Lungo et al.
...
\M{52}This construction is equivalent to making three splices in the
...
\M{53}The canonical permutation representations are now different, because
...
\M{54}The DDP correspondence between an \RNBPM/ $T$ and its
...
\M{55}Here then is a subroutine that implements what was just said.
...
\M{56}The steps of the chart follow preorder.
...
\M{57}If $c$'s right child is just a bud, the subtree $R$ is empty and
...
\M{58}\B\X58:Recursively build the \RNBPM/ for $T$\X${}\E{}$\6
\&{if} (\\{rr})\1\5
${}\\{rnbpm\_ddp}(\|s+\T{1},\39\|c){}$;\2\par
\U55.\fi

\M{59}If \PB{\|c} is the root of the tree, then \PB{\|p} is zero, subtree $%
\widehat S$
...
\M{60}\B\X60:Recursively build the \RNBPM/ for $S$\X${}\E{}$\6
\&{if} (\|p)\1\5
${}\\{rnbpm\_ddp}(\|s+\T{1},\39\|r){}$;\2\par
\U55.\fi

\M{61}Finally we obey the three-step splicing protocol for $\join c$ that was
...
\M{62}Okay, \PB{\\{rnbpm\_ddp}} is finished.
Here's how we apply it to each of the four skew ternary
trees of interest.
...
\M{63}Here, for instance, is the \RNBPM/ that corresponds to the example
...
\M{64}One can show that the number of nodes of even rank is the number
...
\M{65}Indeed, this program led to a huge surprise, because much, much more
...
\M{66}Hey, we're finished---except for a final routine, which we fondly hope
