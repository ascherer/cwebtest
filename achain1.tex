...
\N[2 achain1.w]{1}{1}Intro. This program is a revision of {\mc ACHAIN0}, which
you should
...
\M[64 achain1.w]{2}\B\X2:Process the command line\X${}\E{}$\6
\&{if} ${}(\\{argc}\I\T{3}){}$\5
${}\{{}$\1\6
...
\M[80 achain1.w]{3}\B\X3:Output the value of $l(n)$\X${}\E{}$\6
$\\{fprintf}(\\{outfile},\39\.{"\%c"},\39\|l[\|n]+\.{'\ '});{}$\6
\\{fflush}(\\{outfile});\C{ make sure the result is viewable immediately }\par
\U1.\fi

\M[84 achain1.w]{4}At this point I compute the ``lower bound'' $\lfloor\lg n%
\rfloor+3$,
...
\M[95 achain1.w]{5}Three elementary and well-known upper bounds are considered:
...
\M[122 achain1.w]{6}It's important to try the factor method even when \PB{$\|j%
\Z\T{4}$}, because
...
\N[143 achain1.w]{1}{7}The interesting part.
...
\M[170 achain1.w]{8}We maintain a stack of subproblems, as usual when
backtracking.
...
\M[230 achain1.w]{9}After the test in this step is passed, we'll have \PB{$%
...
\M[250 achain1.w]{10}The undoing mechanism is very simple: When changing \PB{%
\|a[\|j]}, we
...
\M[264 achain1.w]{11}At this point we know that $a[ubp]\le p\le b[ubp]$.
...
\M[294 achain1.w]{12}At this point we had better not assume that $a[ubq]\le q%
\le b[ubq]$,
