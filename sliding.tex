...
\N[2 sliding.w]{1}{1}Introduction. This program solves a fairly general kind of
sliding block
...
\M[10 sliding.w]{2}The puzzle can have up to 15 different kinds of pieces,
named in hexadecimal
...
\M[69 sliding.w]{3}Six different styles of sliding-block moves are supported by
this program,
...
\M[99 sliding.w]{4}OK, here now is the general outline of the program.
...
\M[127 sliding.w]{5}If the style parameter is followed by another parameter on
the command
...
\N[143 sliding.w]{1}{6}Representing the board. An $r\times c$ board will be
represented as
...
\M[169 sliding.w]{7}Every type of piece is specified by a list of board offsets
from the
...
\M[205 sliding.w]{8}\B\D$\\{bufsize}$ \5
...
\M[213 sliding.w]{9}A board position is specified by putting block numbers in
each occupied
...
\M[230 sliding.w]{10}Here is a subroutine that fills the board from a
specification in
...
\M[267 sliding.w]{11}\B\X6:Global variables\X${}\mathrel+\E{}$\6
...
\M[273 sliding.w]{12}The next subroutine prints a given board on standard
output,
...
\M[311 sliding.w]{13}Armed with those routines and subroutines,
...
\M[320 sliding.w]{14}\B\X14:Read the board size\X${}\E{}$\6
...
\M[332 sliding.w]{15}\B\X15:Read the piece specs\X${}\E{}$\6
...
\M[354 sliding.w]{16}\B\X16:Read the starting configuration into \PB{\\{board}}%
\X${}\E{}$\6
...
\M[372 sliding.w]{17}\B\X17:Read the stopping configuration into \PB{%
\\{aboard}}\X${}\E{}$\6
$\\{fgets}(\\{buf},\39\\{bufsize},\39\\{stdin});{}$\6
...
\M[400 sliding.w]{18}\B\X6:Global variables\X${}\mathrel+\E{}$\6
\&{int} \\{balance}[\T{16}];\C{ counters used to ensure that no pieces are lost
}\par
\fi

\N[403 sliding.w]{1}{19}Breadth-first search. Now we're ready for the heart of
the calculation,
...
\M[434 sliding.w]{20}A configuration is represented internally as a sequence of
nybbles
...
\M[476 sliding.w]{23}Conversely, we can reconstruct a board from its packed
representation.
...
\M[498 sliding.w]{24}We use ``universal hashing'' to compute hash codes, xoring
random bits
...
\M[505 sliding.w]{25}The number of hash chains, \PB{\\{hashsize}}, should be a
power of 2,
...
\M[528 sliding.w]{26}\B\X10:Subroutines\X${}\mathrel+\E{}$\6
...
\M[540 sliding.w]{27}Of course I don't expect to keep all configurations in
...
\M[585 sliding.w]{28}The \PB{\\{hashin}} subroutine looks for a given \PB{%
\\{board}} configuration in the
...
\M[614 sliding.w]{29}If the current configuration achieves the goal, \PB{%
\\{hashin}} happily terminates
...
\M[625 sliding.w]{30}\B\X24:Initialize\X${}\mathrel+\E{}$\6
\&{if} (\\{setjmp}(\\{success\_point}))\1\5
\&{goto} \\{hurray};\C{ get ready for \PB{\\{longjmp}} }\2\par
\fi

\M[628 sliding.w]{31}\B\X31:Insert \PB{\\{config}} into the \PB{\\{pos}} table%
\X${}\E{}$\6
...
\M[639 sliding.w]{32}When we encounter a new configuration, we print it if it's
the
...
\M[658 sliding.w]{33}\B\X33:Update \PB{\\{curpos}}\X${}\E{}$\6
...
\M[669 sliding.w]{34}So now we know how to deal with configurations, and we're
ready to
...
\M[688 sliding.w]{35}\B\X35:Remember the stopping configuration\X${}\E{}$\6
...
\M[693 sliding.w]{36}We might need to return to the starting position when
reconstructing
a solution.
...
\M[700 sliding.w]{37}\B\X37:Put the starting configuration into \PB{\\{pos}}%
\X${}\E{}$\6
...
\M[708 sliding.w]{38}\B\X38:Generate all positions at distance \PB{\|d}\X${}%
\E{}$\6
\&{if} ${}(\|d>\T{1}){}$\1\5
...
\N[721 sliding.w]{1}{39}The answer. We've found a solution in \PB{\|d} moves.
...
\M[734 sliding.w]{40}Going backward, we can reconstruct the winning line, as
long as the data
...
\M[754 sliding.w]{41}\B\X41:Apologize for lack of memory and go back to square
...
\N[761 sliding.w]{1}{42}Moving. The last thing we need to do is actually slide
the blocks.
...
\M[771 sliding.w]{43}In the \PB{\\{move}} subroutine, parameter \PB{\|k} is a
block number,
...
\M[805 sliding.w]{44}Style 1 is straightforward: We keep moving in direction %
\PB{\\{delo}} until we
...
\M[859 sliding.w]{45}Local variables \PB{\|s} and \PB{\|t} need not be
...
\N[878 sliding.w]{1}{46}Supermoving. The remaining job is the most interesting
one: How should we
...
\M[906 sliding.w]{47}Elementary graph theory helps now.
...
\M[942 sliding.w]{48}And the problem of generating all superpieces for up/down
slides
...
\M[956 sliding.w]{49}\B\X6:Global variables\X${}\mathrel+\E{}$\6
...
\M[960 sliding.w]{50}The following subroutine for ideals of a digraph maintains
a permutation
...
\M[1013 sliding.w]{51}\B\X51:Put all vertices that lead to \PB{\\{perm}[\|j]}
...
\M[1028 sliding.w]{52}\B\X52:Put all vertices that lead from \PB{\\{perm}[\|j]}
into positions near $j$\X${}\E{}$\6
${}\{{}$\1\6
${}\|u\K\\{perm}[\|j];{}$\6
...
\M[1043 sliding.w]{53}\B\X6:Global variables\X${}\mathrel+\E{}$\6
...
\M[1049 sliding.w]{54}\B\X54:Process an ideal\X${}\E{}$\6
\&{if} (\|p)\5
${}\{{}$\1\6
...
\M[1056 sliding.w]{55}The \PB{\\{supermove}} routine is like \PB{\\{move}}, but
...
\M[1084 sliding.w]{56}After we've moved a superpiece once, the digraph changes
and so do the
...
\M[1101 sliding.w]{57}The program now comes to a glorious conclusion as we put
the remaining
