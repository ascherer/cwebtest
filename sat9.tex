...
\N[4 sat9.w]{1}{1}Intro. This program is part of a series of ``SAT-solvers''
that I'm putting
...
\M[21 sat9.w]{2}If you have already read {\mc SAT8}, or any other program of
this
...
\M[72 sat9.w]{3}So here's the structure of the program. (Skip ahead if you are
...
\M[104 sat9.w]{4}\B\D$\\{show\_basics}$ \5
...
\M[126 sat9.w]{5}On the command line one can specify any or all of the
following options:
\smallskip
\item{$\bullet$}
...
\N[188 sat9.w]{1}{6}The I/O wrapper. The following routines read the input and
absorb it into
...
\M[225 sat9.w]{7}Each clause in the temporary tables is represented by a
sequence of
...
\M[240 sat9.w]{8}\B\X4:Global variables\X${}\mathrel+\E{}$\6
...
\M[255 sat9.w]{9}\B\X9:Initialize everything\X${}\E{}$\6
...
\M[271 sat9.w]{10}The hash address of each variable name has $h$ bits, where
$h$ is the
...
\M[321 sat9.w]{11}\B\X11:Input the clause in \PB{\\{buf}}\X${}\E{}$\6
\&{for} ${}(\|j\K\|k\K\T{0};{}$  ; \,)\5
${}\{{}$\1\6
\&{while} ${}(\\{buf}[\|j]\E\.{'\ '}){}$\1\5
...
\M[342 sat9.w]{12}We need a hack to insert the bit codes 1 and/or 2 into a
pointer value.
...
\M[364 sat9.w]{13}\B\X13:Install a new \PB{\&{vchunk}}\X${}\E{}$\6
${}\{{}$\1\6
...
\M[377 sat9.w]{14}\B\X14:Install a new \PB{\&{chunk}}\X${}\E{}$\6
${}\{{}$\1\6
...
\M[390 sat9.w]{15}The hash code is computed via ``universal hashing,'' using
the following
...
\M[397 sat9.w]{16}\B\X16:Put the variable name beginning at \PB{\\{buf}[\|j]}
...
\M[413 sat9.w]{17}\B\X17:Find \PB{$\\{cur\_tmp\_var}\MG\\{name}$} in the hash
table at \PB{\|p}\X${}\E{}$\6
...
\M[423 sat9.w]{18}The most interesting aspect of the input phase is probably
the ``unwinding''
...
\M[432 sat9.w]{19}An input line that begins with `\.{\~\ }' is silently treated
as a comment.
...
\M[446 sat9.w]{20}\B\X20:Move \PB{\\{cur\_cell}} backward to the previous cell%
\X${}\E{}$\6
...
\M[455 sat9.w]{21}\B\X21:Move \PB{\\{cur\_tmp\_var}} backward to the previous
...
\M[464 sat9.w]{22}\B\X22:Report the successful completion of the input phase%
\X${}\E{}$\6
...
\N[468 sat9.w]{1}{23}SAT solving, version 9. Survey Propagation is slightly
similar to WalkSAT,
...
\M[502 sat9.w]{24}The data structures are analogous to those of previous
programs in this
...
\M[533 sat9.w]{25}\B\X4:Global variables\X${}\mathrel+\E{}$\6
...
\M[541 sat9.w]{26}Here is a subroutine that prints a clause symbolically. It
illustrates
...
\M[556 sat9.w]{27}Another simple subroutine shows the two $\pi$ and $\eta$
...
\N[568 sat9.w]{1}{28}Initializing the real data structures.
...
\M[580 sat9.w]{29}\B\X29:Allocate the main arrays\X${}\E{}$\6
\\{free}(\\{buf});\5
...
\M[607 sat9.w]{30}\B\X30:Zero the links\X${}\E{}$\6
\&{for} ${}(\|l\K\\{vars}+\\{vars};{}$ \|l; ${}\|l\MM){}$\1\5
${}\|o,\39\\{lmem}[\|l-\T{1}].\\{link}\K\T{0}{}$;\2\par
\U28.\fi

\M[610 sat9.w]{31}\B\X31:Copy all the temporary cells to the \PB{\\{mem}} and %
...
\M[629 sat9.w]{32}The basic idea is to ``unwind'' the steps that we went
through while
...
\M[646 sat9.w]{33}\B\X33:Copy all the temporary variable nodes to the \PB{%
...
\M[652 sat9.w]{34}We should now have unwound all the temporary data chunks back
to their
beginnings.
...
\N[665 sat9.w]{1}{35}Doing it. So we take surveys.
...
\M[690 sat9.w]{36}\B\X4:Global variables\X${}\mathrel+\E{}$\6
...
\M[699 sat9.w]{37}The macro \PB{\\{gb\_next\_rand}(\,)} delivers a 31-bit
random integer,
...
\M[713 sat9.w]{38}\B\X38:Compute the $\pi$'s\X${}\E{}$\6
...
\M[725 sat9.w]{39}Either $\eta_l$ or $\eta_{\bar l}$ is zero; the other is
...
\M[777 sat9.w]{40}A clause is ``pseudo-satisfied'' if it contains a variable
whose
...
\M[800 sat9.w]{41}If the clause is $l_1\lor\cdots\lor l_k$, we compute
...
\N[841 sat9.w]{1}{42}The aftermath. When convergence or pseudo-satisfiability
is achieved,
we want to use the
...
\M[865 sat9.w]{43}Here we show not only $\pi_v$ and $\pi_{\bar v}$ for each
variable~$v$,
...
\M[889 sat9.w]{44}\B\X44:Print a two-dimension histogram of $\pi_v$ versus $%
\pi_{\bar v}$\X${}\E{}$\6
${}\{{}$\1\6
...
\M[908 sat9.w]{45}The difference $b=100\,\vert p-q\vert$ in the field of
variable~$v$
...
\M[954 sat9.w]{46}We're done with the \PB{\\{eta}} fields in the clauses of
cells.
...
\M[972 sat9.w]{47}Here now is a subroutine that fixes the variables in a given
bucket list.
...
\M[987 sat9.w]{48}\B\X48:Mark the clauses that contain \PB{\|l} satisfied\X${}%
\E{}$\6
...
\M[993 sat9.w]{49}Removed literals are flagged by a special code in their \PB{%
\\{next}} field.
...
\M[1009 sat9.w]{50}I expect that unit literals will have become sufficiently
biased that
...
\M[1037 sat9.w]{51}\B\X51:Enforce the unit literal \PB{$\\{mem}[\|k-\T{1}].%
\\{lit}$}\X${}\E{}$\6
...
\M[1059 sat9.w]{52}\B\X52:Reduce the problem\X${}\E{}$\6
...
\M[1071 sat9.w]{53}\B\X53:Output the reduced problem\X${}\E{}$\6
...
\M[1099 sat9.w]{54}\B\X4:Global variables\X${}\mathrel+\E{}$\6
