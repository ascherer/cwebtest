...
@*Intro. Given a bidirected graph $G$ on vertices $\{1,2,\ldots,n\}$,
this program uses a variant of dynamic programming
to enumerate all of the Hamiltonian cycles of the induced subgraphs
$G\mid\{1,2,\ldots,m\}$, for $m=1$, 2, \dots,~$n$.
In particular, when $m=n$, it counts {\it all\/} of the Hamiltonian cycles.
And if $G$ is, say, the $8\times20$ knight+wazir graph, with vertices numbered
column by column, it will count all
closed knight+wazir's tours of $8\times5$, $8\times6$, \dots, $8\times20$ boards,
when $m=40$, 48, \dots,~160.
...
This program handles bidirected graphs, which have four types of edges:
extroverted ($u\0<>v$),
indirected ($u\0<<v$),
introverted ($u\0><v$), and
outdirected ($u\0>>v$).
There may be up to four edges between the same two vertices $u$ and~$v$.
...
exactly $2t$ outer cells when there are $t$ subpaths.)  Outer vertices are
``sources'' or ``sinks.'' If a sink vertex $v$ is extended by a
new edge, this edge must point to $v$; that is, it must be either
directed to $v$ (indirected) or extroverted.
Similarly, a new edge that extends a source vertex $v$
must be either directed from $v$ (outdirected) or introverted.

Two $m$-configs
are {\it equivalent\/} if they have the same outer, inner, and bare cells,
the outer cells have the same type (source or sink),
and the outer cells are paired~up in the same way.
...
which is to count the number of $m$-cycles on $\{1,\ldots,m\}$.
...
An inner cell has code~0;
a bare cell has code~1; and
an outer cell of the $j$th subpath has code~$2j+[\rm source]$,
for $1\le j\le t$. The subpaths
are numbered according to when the endpoints first occur, in
a particular ordering of the frontier vertices.

For example, suppose $G$ is the $4\times3$ knight+wazir graph.
There's a knight move between $u$ and~$v$ if and only if
$u\0<>v$; there's a wazir move between $u$ and~$v$ if and only if
$u\0><v$. (A ``wazir'' is an ancient chess piece with only four possible
moves, namely the moves that can be made by both rook and king.)
Instead of naming the vertices 1 thru~12, let's use the cell names
00, 10, 20, 30, 01, 02, \dots, 22, 32.
One of the 4-configs has the following seven edges:
$00\0<>21$,
$00\0><01$,
$10\0<>02$,
$10\0><11$,
...
$20\0><30$,
$30\0<>22$.
So there are $t=2$ subpaths:
$21\0<>00\0><01\0<>20\0><30\0<>22$ and
$11\0><10\0<>02$. Therefore the codes for cells
(01, 11, 21, 31, 02, 12, 22, 32) of the frontier are
respectively (0, 2, 5, 1, 3, 1, 5, 1).
...
@ [{\it Historical note:}\enspace
The first version of this program was written by Peter Weigel in 2025,
based on D.~E. Knuth's
{\mc DYNAHAM} for ordinary graphs. Then Knuth discovered some basic
improvements to the {\mc DYNAHAM} data structures, and incorporated them
into this code with suitable modifications.]
...
@d maxn 300 /* at most this many vertices in the graph */
...
@*Inputting the bidigraph.
An edge between vertices $u$ and $v$ is signified by an entry
with tip~$v$ in the list of edges adjacent to~$u$, and/or by an entry
with tip~$u$ in the list of edges adjacent to~$v$.
It appears twice in this program's data structures, but it need be
specified only once in the input.

As mentioned earlier, it's possible to have up to four edges
between $u$ and $v$  (one of each type). 

Suppose we're in the list of edges adjacent to~$u$. Then
the edge $u\0<<v$ is specified by an arc of length 1 (modulo~4);
the edge $u\0<>v$ is specified by an arc of length 0 (modulo~4);
the edge $u\0><v$ is specified by an arc of length 2 (modulo~4);
the edge $u\0>>v$ is specified by an arc of length 3 (modulo~4).
...
@<Check that the edge lists have the necessary redundancy@>;
printf("Dynamic Hamiltonian cycles of the bidirected graph %s",
                        g->id);
printf(" (%d vertices, %ld edges):\n",
                        n,g->m/2);
fflush(stdout);

@ This code was inspired by Filip Stappers.

@<Check that the edge lists have the necessary redundancy@>=
{
  register Vertex *u,*v;
  register Arc *aa;
  register int alen,aalen;
  for (u=g->vertices;u<g->vertices+n;u++) {
    for (a=u->arcs;a;a=a->next) {
      v=a->tip, alen=a->len;
      if (u==v) {
        fprintf(stderr,"graph has a self-loop %s--%s (length %d)!\n",
                       u->name,v->name,alen);
        exit(-4);
      }
      aalen=alen^((alen&1)<<1); /* the length of the dual arc */
      for (aa=v->arcs;aa;aa=aa->next)
        if (aa->tip==u && (((aa->len^aalen)&0x3)==0)) break; /* dual is there */
      if (!aa) gb_new_arc(v,u,aalen); /* dual was not there, but it is now */
    }
  }
}
...
@ @<Sub...@>=
static char const *encode(int x) 
{static char buf[3]; /* convert class name code to printable string */
  if (x ==0) return("#");
  if (x ==1) return("0");
  buf[2] = 0;
  buf[1] = (x & 1) ? '>' : '<';  /* '\.>' for sources, '\.<' for sinks */
  x >>= 1;
  buf[0] = x + (x < 10 ? '0' : ('a'-10));
  return(buf);
}  
...
@ @<Abort, because bignum overflow has occurred@>=
{
  fprintf(stderr,"\nSorry, I can't handle numbers bigger than 10^%d!\n",
                       18*maxprec);
  fprintf(stderr,"I had found %lldd classes of %d-configs so far.\n",
                         (long long)wtptr,m);
  exit(-999);
}
...
$0\le a_l<2|deg|$ for $0\le l<q$. There is a node in the trie
...
an array of approximately twenty pointers, each of which was either zero or the
...
when all but the last code of the key are known, there are just
two choices for the final code $a_{q-1}$; consequently all but two of the
pointers in the nodes at level~$q-1$ were zero!
...
For instance, $a_0$ itself can only be 0, 1, 2, or~3.
And the key prefix 2053, say, can only be followed by
0, 1, 4, 5, 6, or 7.

We can govern the set of possibilities for code $a_l$ by maintaining
a sparse-set structure: The endpoint codes that have already appeared
once but not twice are listed in the first |tms| elements of the array
|tmap|; and |tmx| is the smallest endpoint code that has not yet appeared.
Furthermore there's a partial inverse map: If $2c$ or $2c+1$ has occurred once
...
values of $a_l$ are respectively 0, 1, 2|tmap[0]|, \dots, $2|tmap|[tms-1]+1$,
2|tmx|, and $2|tmx|+1$, the child node $p'=p(a_0\ldots p_l)$ will be respectively
|mem[p]|, |mem[p+1]|, \dots, |mem[p+2*tms+3]|.
...
0 and~1 cannot occur, as well as cases where 2|tmx| and $2|tmx|+1$ cannot occur.)
...
@d memtyp unsigned int
@d memsize 3000000000LL /* max mem size; must be sufficiently large */
@d wtsize   500000000LL /* ditto for the max number of classes */
@d oldmemsize 450000000 /* see above; this is generous */
@d deg 15 /* the number of subpaths per class must be less than this */

@<Initialize@>=
mem=(memtyp*)malloc(memsize*sizeof(memtyp)); /* nonleaf pointers */
...
@ @<Glob...@>=
memtyp oldp; /* which old weight are we working on? */
long long contribs; /* how many contributions were made by the old classes? */
memtyp *mem; /* the array where the big trie pointers live */
unsigned long long *oldmem; /* the array where compressed bitstrings live */
memtyp memptr; /* the first unused slot in |mem| */
memtyp wtptr; /* the first unused slot in |weight| */
...
bignum count[maxn+1]; /* how many Hamiltonian $m$-cycles have we counted? */
int maxdeg; /* largest observed number of subpaths so far */
unsigned long long pack; /* bits being assembled to append to |oldmem| */
int spack; /* the number of unsaved bits currently in |pack| */
int owp,omp; /* weight and memory pointers when compressing/uncompressing */
long long maxmemptr,maxwtptr,maxomp; /* the largest seen so far */
...
At level |l|, when we branch on $a_l=|code[l]|$, the legal endpoint
...
memtyp trielookup(void) {
  ...
  for (l=0;l<q;l++,p=mem[pp]) {
    j=code[l];
    if (j/2==0) pp=p+j;
    else if (j/2==tmx) { /* a higher code than seen before */
      @<Swap |tmx| into the |tmap| and branch to it@>;
    }@+else { /* code |j/2| matches a previous endpoint */
      @<Remove |j/2| from the |tmap| and branch to it@>;
    }
...
if (tmx>maxdeg) {
  maxdeg=tmx;
  if (tmx>=deg) {
    fprintf(stderr,"Overflow: number of subpaths must be less than %d!\n",
                           deg);
    exit(-66);
  }
}
...
{
  register int slots;
  if (l+1+tms<q) { /* allocate space for 0 and 1 */
    mem[memptr]=mem[memptr+1]=0,memptr+=2; /* this is safe, see below */
    slots=tms+(l+2+tms==q? 0: 1); /* see above */
  }@+else slots=tms; /* allocate space for existing endpoint codes only */
  if (memptr+2*slots+1>=memsize) {
    fprintf(stderr,"Oops: Dictionary overflow (more than %lld pointers)!\n",
                                         memsize@t)@>);
    exit(-666);
  }
  mem[pp]=memptr-2;
  for (j=0;j<slots;j++) mem[memptr+2*j]=mem[memptr+2*j+1]=0;
  memptr+=2*slots; /* now |memptr+1| is still less than |memsize| */
}
...
This subroutine is invoked with |l=0|, |p=0|, |d=4|
...
void compress(int l,memtyp p,int d) {
  ...
  if (l==q) @<Save |weight[p-1]|@>@;
    ...
    for (j=(l+oms==q? 2:0),k=0,bits=0;k<d;j++,k++)
      if (mem[p+j]) bits+=1LL<<k;
    @<Append |bits| to |oldmem|@>;
    for (j=(l+oms==q? 2:0),k=0;k<d;j++,k++) if (bits&(1LL<<k)) {
      if (j/2>0) {
        if (j/2>oms) omap[oms]=omx, iomap[omx]=oms, oms++, omx++, kk=0;
        else kk=omap[--oms],kkk=omap[j/2-1],omap[j/2-1]=kk,iomap[kk]=j/2-1;
      }
      compress(l+1,mem[p+j],2*(oms+(l+1+oms==q? 0:
                               l+2+oms==q? 1: 2)));
      if (j/2>0) { /* we must undo our changes to the sparse-set map */
        if (!kk) oms--,omx--;
        else omap[j/2-1]=kkk,omap[oms]=kk,iomap[kk]=oms++;
      }
    }
  }
}
...
void uncompress(int l,int d) { /* uncompress a node of degree |d| at level |l| */
  ...
  if (l==oldq) {
    ...
    for (j=(l+oms==oldq? 2:0),k=0;k<d;j++,k++) if (bits&(1LL<<k)) {
      if (j/2==0) oldcode[l]=j;
      else if (j/2>oms)
        oldcode[l]=2*omx+(j&1),omap[oms]=omx,iomap[omx]=oms,omx++,oms++,kk=0;
      else oldcode[l]=2*omap[j/2-1]+(j&1),kk=omap[--oms],
                     kkk=omap[j/2-1],omap[j/2-1]=kk,iomap[kk]=j/2-1;
      uncompress(l+1,2*(oms+(l+1+oms==oldq? 0:
                          l+2+oms==oldq? 1: 2)));
      if (j/2>0) { /* we must undo our changes to the sparse-set map */
        if (!kk) oms--,omx--;
        else omap[j/2-1]=kkk,omap[oms]=kk,iomap[kk]=oms++;
...
$\vert F_m\vert\le n-m$.

For our purposes it turns out to be better to work with the {\it extended\/}
frontiers $\widehat F_m=F_m\cup\{m+1\}$, for $0\le m<n$, so that $m+1$ is
always an element of~$\widehat F_m$. In particular, $\widehat F_0=\{1\}$.
This change doesn't change the definition
of equivalence classes, because vertex $m+1$ will always be bare if it
wasn't already in $F_m$. And we'll see that extended frontiers make
the program simpler.
...
More precisely, we have |fr[ifr[v]]=v|, for $1\le v\le n$, and
$1\le|fr[k]|\le n$ for $0\le k<n$. Vertex $v$ is part of $\widehat F_m$ if and
...
use the names $\{1,\ldots,n\}$ for the vertices, instead of calling
them $\{0,\ldots,n-1\}$.)
...
Then we will arrange things so that
|fr[k-1]<fr[k]| for $0<k<q_0$ and for $q_0<k<q$. In particular, we'll always
have |fr[0]=m+1|.
...
q=1;
mem[0]=0; /* no inner vertices */
mem[1]=1; /* one bare vertex (1) */
mem[2]=0; /* no first sink */
mem[3]=0; /* no first source */
memptr=4; /* four pointers in the root node */
weight[0]=one,wtptr=1; /* one lief */
...
if (iv<--q) { /* it's in $F_{m-1}$ and not the last guy */
  ...
  fr[0]=m+1,ifr[m+1]=0,fr[iv]=m,ifr[m]=iv; /* swap |m| with |m+1| */
...
for (t=0,k=1;k<=q;k++) {
  j=mate[k];
  if (j<=0) code[k-1]=j + 1; /* $\rm inner=0, bare=1$ */
  else if (j>k) 
       {t += 2;
        code[k-1]=t + src[k];
        code[j-1]=t + src[j];
...
for (k=1;k<=oldq;k++) {
  j=oldcode[k-1];
  if (j<=1) oldmate[k]=j - 1; /* inner or bare */
  else {
    oldsrc[k] = j & 1; /* split code into src-flag and path number */
    j >>= 1;
    l=path[j];
    if (!l) path[j]=k;
...
int src[maxn],oldsrc[maxn]; /* mate or oldmate source or sink */
...
int nbr[maxn*4];     /* list of those neighbors, as indices to the frontier, 4 different edges possible */
int nbrtype[maxn*4];
 /* edge type to neighbor: $\rm 0=extro, 1=indir, 2=intro, 3=outdir$ */
...
Consider therefore the case when $m=5$ and the 4-frontier of the graph
is the sequence of vertices $(5,6,13,7,8)$. Furthermore we shall assume
that the neighbors of vertex~5 that exceed~5 are 6, 8, 11, and 12.

It follows from the program above that the 5-frontier of the graph
will be the sequence of vertices $(6,7,8,13,11,12)$. (First the
|fr| array is changed so that its first five elements are $(6,8,13,7,5)$,
and $q$ is reduced from |oldq=5| to~$q_0=4$;
then 11 and 12 are swapped into place while $q$ increases to~6;
then we sort.)

In the trie of 4-classes (the equivalence classes for 4-configs),
the vertices $u_1u_2u_3u_4u_5$ are the vertices 5,~6, 13, 7, 8 of the graph,
in that order;
and the |oldmate| table uses indices $\{1,2,3,4,5\}$ for those vertices.
For example, if vertices 6 and 7 are endpoints of a subpath in some
equivalence class, they are vertices $u_2$ and $u_4$; hence
|oldmate[2]=4| and |oldmate[4]=2|.

The vertices $v_1v_2v_3v_4v_5v_6$ of the 5-frontier are, similarly,
vertices 6, 7, 8, 13, 11, 12 of the graph,
in that order. Vertices 6 and 7 of the graph
are now $v_1$ and $v_2$; so the relations |oldmate[2]=4| and |oldmate[4]=2|
in the 4-frontier 
are equivalent to |mate[1]=2| and |mate[2]=1| in the 5-frontier.

It's natural therefore to construct a |map| array that characterizes the
relation between the two labelings: |map[1]=0|, |map[2]=1|, |map[3]=4|,
|map[4]=2|, |map[5]=3|, so that |map[1]| is always zero and
$u_j=v_{map[j]}$ for $1<j\le|oldq|$.
We also set |map[-1]=-1| and |map[0]=0|.

Furthermore we can make an inverse array |imap|, with
|imap[1]=2|, |imap[2]=4|, |imap[3]=5|, |imap[4]=3|; in general
...
|oldmate[imap[j]]=1|,'' because |map[1]=0|. Let's ignore that exceptional
case for the moment.
...
is ``inner'' in its $(m-1)$-config, namely when |oldmate[1]=-1|.
...
for (;j<=q;j++) bmate[j]=0; /* |bmatesrc[j]| is irrelevant */
...
$(6,8,11,12)$, also happen to be $(v_1,v_3,v_5,v_6)$. In general when vertex~$m$
has $r$ neighbors in the $m$-frontier, we can list their indices in
the |nbr| array; our example has $r=4$, |nbr[0]=1|, |nbr[1]=3|, |nbr[2]=5|,
|nbr[3]=6|.
...
is ``bare,'' namely when we have |oldmate[1]=0|.
Then there are $r\choose2$ potential contributions
to $m$-classes, namely one for every pair of neighbors.
In our example, one of the ${4\choose2}=6$ choices of neighbors is
$v_3\adj 5\adj v_6$; we add those two edges to the class for |bmate| if we can.
The effect is the same as adding the derived edge $v_3\adj v_6$ to that class.

(Well, we actually need to match bidirectionally. Nothing is done if, say,
$v_3\0<>5\0<<v_6$. And if we have a valid pair of edges like
$v_3\0<>5\0><v_6$, the derived edge is $v_3\0<<v_6$.)

@<Contribute the relevant classes when |m| is bare@>=
{
  for (i=0;i<r;i++) 
    for (ii=i+1;ii<r;ii++) {
       if ((nbrtype[i] & 2) == 0) { /* neighbor |i| is extro or indir */
         /* neighbor |ii| must be intro or outdir */
         if ((nbrtype[ii] & 2) == 0) continue;
         srci   = (nbrtype[i] & 1) ^ 1;
             /* next edge must be outgoing for extro or incoming for indir */
         srcii = (nbrtype[ii]& 1);
            /* next edge must be outgoing for outdir or incoming for intro */
       } else { 
         /* neighbor |i| is intro or outdir */
         /* so neighbor |ii| must be extro or indir */
         if ((nbrtype[ii] & 2) != 0) continue;
         srci   = (nbrtype[i] & 1);
           /* next edge must be outgoing for outdir or incoming for intro */
       ...
       if (add_derived(nbr[i],srci,nbr[ii],srcii)) contribute();
    }
}

@ Finally, the exceptional cases arise when |oldmate[1]>0|. Then $r$
contributions to $m$-classes occur, one for each neighbor.
For example, suppose |oldmate[1]=5|, meaning that $u_1$ is the
endpoint of a path whose other endpoint is $u_5$, which is also
$v_{map[5]}=v_3$. (It's an exceptional case because
|oldmate[imap[3]]=oldmate[5]=1|.) Adding an edge from $u_1$ to $v_j$ is
very much like adding an edge between $v_3$ and $v_j$, because
it creates a path from $v_3$ to $v_j$.

@<Contribute the relevant classes when |m| is outer@>=
{
  for (i=0;i<r;i++) {
    if (oldsrc[1]) {
        /* must be outgoing: intro or outdir */
      if ((nbrtype[i] & 2) == 0) continue;
      srci = nbrtype[i] & 1; 
             /* next edge must be outgoing for outdir or incoming for intro */
    } else {
      /* must be incoming: extro or indir */
      if ((nbrtype[i] & 2) != 0) continue;
      srci = (nbrtype[i] & 1) ^ 1;
       /* next edge must be outgoing for extro or incoming for indir */
    }
    ...
    if (add_derived(mp[1+oldmate[1]],oldsrc[oldmate[1]],nbr[i],srci))
       contribute();
  }
}

@ If vertex $m+1$ is not in the old frontier, we want |imap[1]| to be zero,
so that |bmate[1]| will also be zero.
Otherwise |imap[1]| should be set to the former location of $m+1$.
...
If |srci=1|, $v_i$ becomes a source vertex which needs an outgoing edge.
Otherwise it needs an incoming edge. Same for |srcj| and $v_j$.

And in the latter case there are four subcases, depending on whether
$v_i$ and/or $v_j$ are bare. For example, suppose $v_i$ is bare and
$v_j$ is outer. Then the bidirectional rules tell us that
|srcj=src[j]| is illegal.
...
@<Sub...@>=
int add_derived(int i, int srci, int j, int srcj) {
  register int k,kk;
  ...
  if (!mate[j]) { /* $v_j$ is bare */
    if (src[i] == srci) return 0;
     mate[j]=mate[i],mate[mate[i]]=j,mate[i]=-1; /* $v_i$ becomes inner */
     src[j] = srcj;
     return 1;
  }
  /* $v_i$ and $v_j$ both outer */
  if (i==j) return 0; /* three edges at $v_i$ */
  if (src[i]==srci || src[j]==srcj) return 0;
  if (mate[i]!=j) { /* join two subpaths */
...
are satisfied, we contribute to the count of all Hamiltonian $m'$-cycles.
...
|fr[0]=m+1|, |fr[1]=m+2|, \dots, |fr|$[m'-m-1]=m'$, and $m'-m\le q_0$.
...
cycle: mate[i]=mate[j]=-1;
  for (k=1;k<=q0;k++) {
    ...
    if (fr[k-1]!=m+k) return 0; /* |m+k| is not inner */
  }
  for (kk=k;kk<=q;kk++)
    if (mate[kk]) return 0; /* a frontier element |>=m+k| isn't bare */
  {
    register int l;
    fprintf(stderr,"Class ");
    for (l=0;l<oldq;l++) fprintf(stderr,"%s",
                         encode(oldcode[l]));
...
@ @<Visit the class |oldcode| with weight |oldweight[oldp]|@>=
{
  ...
  @<Create the |bmate| table from |oldmate|@>;
  if (oldmate[1]<0) @<Contribute the |bmate| class@>@;
  else if (oldmate[1]==0) @<Contribute the relevant classes when |m| is bare@>@;
...
mem[0]=mem[1]=mem[2]=mem[3]=0,memptr=4,wtptr=0;
...
@ @<Sub...@>=
void report_cycles(int m) { /* report the number of Hamiltonian $m$-cycles */
