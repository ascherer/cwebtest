...
\N[4 obdd.w]{1}{1}Introduction. This program counts the number of knight's
tours of an
...
\M[38 obdd.w]{2}To account for $180^\circ$ symmetry, we identify each vertex
with its
...
\M[69 obdd.w]{3}\B\X3:Global variables\X${}\E{}$\6
...
\M[76 obdd.w]{4}\B\X4:Local variables\X${}\E{}$\6
...
\N[81 obdd.w]{1}{5}OBDDs. An OBDD canonically represents a boolean function
...
\M[110 obdd.w]{6}To get started, I set up a simple OBDD for the function $f$
that says
...
\M[143 obdd.w]{7}Here's a subroutine for use when debugging: It prints the
current OBDD.
...
\M[155 obdd.w]{8}To complete the construction of the OBDD for matching, we need
to specify
...
\M[166 obdd.w]{9}The reader may have noticed that I forgot to test whether \PB{%
\\{curnode}} has
...
\M[187 obdd.w]{10}\B\X4:Local variables\X${}\mathrel+\E{}$\6
\&{int} \|f${},{}$ \|g;\C{ roots of OBDDs }\par
\fi

\N[190 obdd.w]{1}{11}Intersection of OBDDs. Now comes the funnest part. Given
the roots $f,g$ of
...
\M[220 obdd.w]{12}\B\X3:Global variables\X${}\mathrel+\E{}$\6
\&{int} \\{hinode};\C{ the first free node in upper memory }\par
\fi

\M[223 obdd.w]{13}What's a template? Well, it's sort of like an OBDD except
that it hasn't
...
\M[257 obdd.w]{14}The \PB{\\{new\_template}} routine recognizes previous
...
\M[273 obdd.w]{15}\B\X15:Initialize the tables for template construction\X${}%
\E{}$\6
...
\M[277 obdd.w]{16}I forgot to mention that
...
\M[305 obdd.w]{17}The second phase of \PB{\\{intersect}} uses a routine \PB{%
\\{new\_node}} that is
...
\M[329 obdd.w]{18}OK, we're ready to finish off the intersection process. The
idea is to go
...
\M[360 obdd.w]{19}The \PB{\\{intersect}} routine is now complete. I just want
...
\N[368 obdd.w]{1}{20}Counting the matchings. One of the neatest properties of
the OBDD is that
...
\M[382 obdd.w]{21}\B\X21:Count and report the number of such matchings\X${}%
\E{}$\6
...
\N[390 obdd.w]{1}{22}Hamiltonicity. The first two edges in our list are the two
knight moves
...
\M[421 obdd.w]{23}When I got to the point of writing this part of the program,
it became clear
...
\M[471 obdd.w]{24}In this part of the program I'm implementing recursive
...
\M[499 obdd.w]{25}\B\X4:Local variables\X${}\mathrel+\E{}$\6
\&{int} \\{tt};\C{ node being traversed in the outer loop }\par
\fi

\M[502 obdd.w]{26}\B\X3:Global variables\X${}\mathrel+\E{}$\6
...
\M[507 obdd.w]{27}The inner traversal is very similar, except that we
...
\M[531 obdd.w]{28}\B\X28:Record a solution\X${}\E{}$\6
...
\N[550 obdd.w]{1}{29}Experiences. When I ran this program in May, 1996, I was
able to
