...
\M{3}In the following program, the parent of node $k$ is \PB{\\{par}[\|k]}, and
the
...
\M{6}Consider the example spider
$$\epsfbox{deco.5}$$
...
\M{7}We don't want to compute the sets $U_1$, \dots, $U_n$ explicitly,
...
\M{8}Tree traversal is great fun, when it works.
...
\M{9}The sample spider leads, for example, to the following values:
$$\vbox{\halign{$\hfil#\hfil$\quad&
$\hfil#\hfil$\quad&
$\hfil#\hfil$\quad&
$\hfil#\hfil$\quad&
...
\M{11}Another thing we'll need to know is \PB{\\{umaxbit}[\|k]}, the value of
...
\M{12}For the record, our example spider has the following additional
...
\M{13}A somewhat subtle point arises here, and it provides an important
...
\N{1}{14}The active list. Reflected Gray code is nicely generated by a process
...
\M{15}Using the active list protocol, the average amount of work per bit change
is
...
\M{16}Now let's focus on the implementation of step (2), which is the
...
\M{17}\B\X17:Compute the \PB{\\{bstart}} links for \PB{\|k}'s family\X${}\E{}$\6
...
\M{18}\B\X4:Global variables\X${}\mathrel+\E{}$\6
...
\M{19}When \PB{\\{bit}[\|k]} changes from 0 to 1, we want to delete $k$'s
positive blocks of
...
\M{20}Once the process has gotten started, \PB{\\{left}[\|j]} and \PB{%
\\{right}[\|k]} will
...
\M{21}A block being deleted might be preceded by a simple block of the
...
\M{22}\B\X22:Replace block $k$ by block $i$ and \PB{\&{return}}\X${}\E{}$\6
${}\{{}$\1\6
...
\M{23}How does the active list get there in the first place? We
...
\N{1}{24}Doing it. The time has come to construct the loopless implementation
in
...
\M{26}\B\X26:Set \PB{\|k} to the rightmost nonsleeping node of the active list%
\X${}\E{}$\6
$\|j\K\\{left}[\T{0}],\39\|k\K\\{focus}[\|j],\39\\{focus}[\|j]\K\|j{}$;\par
\U24.\fi

\M{27}At this point we know that all nodes greater than $k$ are awake and
...
\M{28}\B\X28:Move forward, setting \PB{$\\{bit}[\|k]\K\T{1}$}\X${}\E{}$\6
${}\{{}$\1\6
...
\M{29}\B\X29:Move backward, setting \PB{$\\{bit}[\|k]\K\T{0}$}\X${}\E{}$\6
${}\{{}$\1\6
...
\M{30}\B\X30:Print out all the current bits\X${}\E{}$\6
${}\{{}$\1\6
...
\M{31}Here I recompute what the active list should be, and compare it
...
\M{33}Finally, we print even more stuff when the user calls for an
