...
@*Intro. This program finds all the Hamiltonian cycles of a given
bidirected graph.
...
I've based it on {\mc SSDIHAM}, which is similar but for
ordinary directed graphs.

\def\0#1#2{\mathrel{\.{#1#2}}}
Bidirected graphs were introduced by Jack Edmonds and Ellis L. Johnson
in 1970, but they still aren't known as well as they deserve to be.
Each edge between vertices $u$ and $v$ is denoted by two directional
symbols, either \.< or \.>; hence there are four possibilities:
\smallskip
$u\0<<v$ (``directed edge from $u$ to $v$'');
\smallskip
$u\0<>v$ (``extroverted edge between $u$ and $v$'');
\smallskip
$u\0><v$ (``introverted edge between $u$ and $v$'');
\smallskip
$u\0>>v$ (``directed edge from $v$ to $u$'').
\smallskip\noindent
(Actually there are only three really distinct possibilities, because
$u\0<<v$ is the same as $v\0>>u$.)

A {\it path\/} of length $k$ in a bidirected graph is a sequence
of $k$ edges where the intermediate vertices are surrounded by
identical directions. In other words, when the path begins with
$u\0<<v$ or $u\0><v$, the next edge must have the form
$v\0<<w$ or $v\0<>w$. Similarly, an edge of the form
$u\0<>v$ or $u\0>>v$ must be followed by either
$v\0><w$ or $v\0>>w$. Here's a cycle of length four,
which exhibits all four types of edges:
$$w\0<<x\0<>y\0>>z\0><w.$$
...
a bidirected graph~|g| and removing edges until only a
bidirected cycle is left. 

There's a simple way to convert any bidirectional graph $B$
to an undirected graph~$G$,
where $B$ and~$G$ contain exactly the same number of Hamiltonian cycles:
Each vertex~$v$ of~$B$ leads to three vertices $\{v^-,v,v^+\}$ of~$G$;
each edge $u\0<<v$ of~$B$ leads to an edge $u^-\adj v^+$ of~$G$;
each edge $u\0<>v$ of~$B$ leads to an edge $u^-\adj v^-$ of~$G$; and
each edge $u\0><v$ of~$B$ leads to an edge $u^+\adj v^+$ of~$G$.
The graph~$G$ also contains two edges $v^-\adj v\adj v^+$ for each
vertex~$v$ of~$B$. Clearly $B$ contains the four-edge oriented cycle
of the introduction if and only if $G$ contains the cycle
$$w^+\adj w\adj w^-\adj x^+\adj x\adj x^-\adj y^-\adj y\adj y^+
\adj z^-\adj z\adj z^+\adj w^+.$$

With that construction, we could reduce the bidirected graph problem faced by
{\mc SSBIDIHAM} to the graph problem already solved by {\mc SSHAM}.
But we can do better: (i) We need only work with the signed
vertices $v^-$ and~$v^+$,
not with~$v$; and (ii) we can force an edge between signed vertices
to be in the cycle whenever either of its endpoints
has degree~1 in the remaining graph.

Thus we shall regard the user's $n$-vertex bidirected graph as a $2n$-vertex
...
Notice that, in |g|, the degree of vertex $v^-$ is the number of
edges directed from~$v$ plus the number of extroverted edges
adjacent to~$v$ in~$B$. The degree of vertex~$v^+$ is the number of edges
directed to~$v$ plus the number of introverted edges adjacent to~$v$ in~$B$.
...
opposite directions. (For example, the edge $u^+\adj v^+$ is actually
...
|u| and~|v|. If the $k$th chosen edge is $s\adj t$,
we have either $e[k].u=s$ and $e[k].v=t$ or
               $e[k].u=t$ and $e[k].v=s$,
...
@ Let $\bar s$ denote vertex $s$ with the opposite sign.
(Thus, if $s=v^-$ we have $\bar s=v^+$, and vice versa.)
If the edges chosen so far include a maximal subpath of the form
$$\bar s_0\adj s_1,\ \bar s_1\adj s_2, \ \dots, \ \bar s_{k-1}\adj s_k,$$
where $s_0$ and $\bar s_k$ don't participate in any other chosen edge,
we say that $s_0$ and $\bar s_k$ are ``outer'' vertices,
while $\bar s_0$, $s_1$, $\bar s_1$, \dots, $\bar s_{k-1}$, $s_k$ are ``inner.''
...
above, we have $|mate|(s_0)=\bar s_k$ and $|mate|(\bar s_k)=s_0$;
that rule defines
...
@ The Stanford GraphBase was designed to deal with undirected graphs
and directed graphs, but not bidirected graphs. So we need to
specify how a bidirected graph should be represented.

This program is experimental --- it's my first attempt to handle
bidirectionality --- and I've decided for now to define the
representation in the following slightly peculiar way.
An edge between vertices $u$ and $v$ is signified by an entry
with tip~$v$ in the list of edges adjacent to~$u$, or by an entry
with tip~$u$ in the list of edges adjacent to~$v$, or both.
(If both are given, they must agree.) It's possible to have
up to four edges between $u$ and $v$ -- one of each type.
...
if (mind<1) {
  printf("There are no Hamiltonian cycles, because "O"s"O"c has degree 0!\n",
        name(curv));
  exit(0);
}
fprintf(stderr,"OK, I've got a bidigraph with "O"d vertices, "O"d edges,\n",
                                nn,mm);
...
for (mm=v=0;v<nn;v++) {
  register int up,vp,d,dd,uu,vv,l;
  register Arc *a;
  rmems++,vp=perm[v];
  oo,mate(vp+vp)=mate(vp+vp+1)=-1;
  for (o,a=(g->vertices+v)->arcs;a;o,a=a->next) {
    o,u=a->tip-g->vertices,l=a->len;
    if (u==v) {
    ...
    rmems++,up=perm[u];
    switch (l&0x3) {
case 0: vv=vp+vp,uu=up+up;@+break;
case 1: vv=vp+vp,uu=up+up+1;@+break;
case 2: vv=vp+vp+1,uu=up+up+1;@+break;
case 3: vv=vp+vp+1,uu=up+up;@+break;
    }
    if (adj[vv][uu]==infty) { /* not already seen */
      o,d=deg(vv);
      oo,nbr[vv][d]=uu;
      oo,adj[vv][uu]=d;
      o,deg(vv)=degree[vv]=d+1;
      o,dd=deg(uu);
      oo,nbr[uu][dd]=vv;
      oo,adj[uu][vv]=dd;
      o,deg(uu)=degree[uu]=dd+1;
      mm++;
    }
  }
}
if (randomizing) {
  for (v=0;v<2*nn;v++) {
    for (o,j=1;j<deg(v);j++) {
      mems+=4,k=gb_unif_rand(j+1);
      oo,u=nbr[v][j],w=nbr[v][k];
      oo,nbr[v][j]=w,nbr[v][k]=u;
      oo,adj[v][w]=j,adj[v][u]=k;
    }
  }
...
@ A subtle point ought to be explained here: Suppose the bidirected input graph
simply has two vertices $\{0,1\}$
and two arcs: $0\0<<1\0<<0$. Then the graph by which we represent it
...
@<Unscramble and print the current solution@>=
{
  ...
  for (k=0;k<=nn;k++) printf(""O"s ",basename(path[k]));
  printf("#"O"llu\n",count);
}
