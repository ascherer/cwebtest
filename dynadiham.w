...
@*Intro. Given a directed graph $G$ on vertices $\{1,2,\ldots,n\}$,
...
arcs of~$G$ such that (i)~every vertex $\le m$ appears in exactly two arcs,
one of which enters~$v$ and the other leaves~$v$;
(ii)~no arc has both endpoints $>m$;
(iii)~there is no oriented cycle of arcs.
As a consequence, the arcs of an $m$-config will form disjoint oriented subpaths
of the graph.

The ``$m$-frontier'' $F_m$ of $G$ is the set of vertices $>m$ that are
reachable from $\{1,\ldots,m\}$ via arcs in either direction.
Each vertex of~$F_m$ in an $m$-config can
...
``sources'' or ``sinks''; every subpath runs from a sink to a source.
(This terminology is perhaps backwards from what you might expect. But
we want to extend a subpath by adding a new arc to its sink, or
by adding a new arc from its source.)
...
This program is a modification of the program {\mc DYNABIDIHAM},
written by Peter Weigel and Don Knuth in 2025.

Cycles in directed graphs are special cases of cycles in bidirected
graphs; so the underlying algorithm is essentially the same.
No attempt has been made to simplify that underlying algorithm.
However, we can save almost half of the memory, because bidirected
graphs allow subpaths between source and source or between sink and sink,
in addition to the ordinary one-way variety; we no longer need to
provide space to remember such impossible states.

Another difference between this program and {\mc DYNABIDIHAM} is that
the input can be any graph in the standard Stanford GraphBase format.
Bidirected graphs, by contrast, need a special encoding in the
length fields of their arcs/edges.
...
@*Inputting the digraph as a bidigraph.
An arc from vertex $u$ to vertex $v$ is signified by an entry
with tip~$v$ in the list of arcs adjacent to~$u$.

This program's data structures also represent the {\it converse\/}
arc, from vertex $v$ back to vertex~$u$, by putting an entry
with tip~$u$ into the list of arcs adjacent to~$v$.

To distinguish a ``real'' arc from a ``converse'' arc, we use the
length field |len| in a Stanford GraphBase \&{Arc} record:
A real arc of length $l$ in the input graph is converted to an
arc of length $4l+1$, and its converse is stored as an arc of
length $4l+3$. (The original length |l| has no effect on the behavior
of this program, but we retain it just in case an extended program
might want to see it.)

This duplication of arcs, with corresponding changes to  the |len| fields,
means that we've internally converted the
given directed graph to a bidirected graph. Therefore the
algorithm of {\mc DYNABIDIHAM} applies to it.
...
@<Convert the arc lists to bidirected form@>;
printf("Dynamic Hamiltonian cycles of the directed graph %s",
                        g->id);
printf(" (%d vertices, %ld arcs):\n",
                        n,g->m/2);
fflush(stdout);
...
@ @<Convert the arc lists to bidirected form@>=
{
  ...
  for (u=g->vertices;u<g->vertices+n;u++) {
    for (a=u->arcs;a;a=a->next) {
      v=a->tip;
      if (u==v) {
        fprintf(stderr,"graph has a self-loop %s--%s (length %ld)!\n",
                       u->name,v->name,a->len);
        exit(-4);
      }
      a->len=4*a->len+1;
  ...
  for (u=g->vertices;u<g->vertices+n;u++) {
    for (a=u->arcs;a;a=a->next) {
      v=a->tip;
      if ((a->len&0x3)==1) gb_new_arc(v,u,a->len+2);
    }
  }
}
...
0, 1, 4, 6, or 7. (It can't be 2 or 3, because subpath~1 has
codes $\{2,3\}$ and we've already seen them both. It can't be 5,
because subpath~2 has codes $\{4,5\}$ and we've already seen~5.)
...
In order to doublecheck our logic, we also remember |tmiss[c]|,
its missing appearance.
...
values of $a_l$ are respectively 0, 1, |tmiss[tmap[0]]|, \dots,
|tmiss[tmap[tms-1]]|,
2|tmx|, and $2|tmx|+1$, the child node $p'=p(a_0\ldots p_l)$ will be respectively
|mem[p]|, |mem[p+1]|, \dots, |mem[p+tms+3]|.
...
int tmiss[deg],omiss[deg]; /* $2c$ or $2c+1$, whichever hasn't occurred */
...
tmap[tms]=tmx, itmap[tmx]=tms, tmiss[tmx]=j^1;
tms++,tmx++;
pp=p+1+tms+(j&1);
...
@ Some subtlety here: We don't provide a slot for 2|tmx| and $2|tmx|+1$
when |l+2+tms=q|,
because the endpoint |tmx| will have to occur twice.
...
@<Allocate a new node for level |l+1|@>=
{
  ...
  if (l+1+tms<q) { /* allocate space for 0 and 1 */
    ...
    slots=tms+(l+2+tms==q? 0: 2); /* see above */
  ...
  if (l==q) @<Save |weight[p-1]|@>@;
    ...
    for (j=(l+oms==q? 2:0),k=0;k<d;j++,k++) if (bits&(1LL<<k)) {
      if (j>=2) {
        ...
        else kk=omap[--oms],kkk=omap[j-2],omap[j-2]=kk,iomap[kk]=j-2;
      }
      compress(l+1,mem[p+j],oms+(l+1+oms==q? 0:
                               l+2+oms==q? 2: 4));
      if (j>=2) { /* we must undo our changes to the sparse-set map */
        if (!kk) oms--,omx--;
        else omap[j-2]=kkk,omap[oms]=kk,iomap[kk]=oms++;
      }
    }
  }
}
  ...
  if (l==oldq) {
    ...
    for (j=(l+oms==oldq? 2:0),k=0;k<d;j++,k++) if (bits&(1LL<<k)) {
      if (j<2) oldcode[l]=j;
        ...
        omiss[omx]=2*omx+3+oms-j, oldcode[l]=omiss[omx]^1;
        omap[oms]=omx,iomap[omx]=oms,omx++,oms++,kk=0;
      }@+else kk=omap[--oms],kkk=omap[j-2],omap[j-2]=kk,iomap[kk]=j-2,
                   oldcode[l]=omiss[kkk];
...
where $u\adj v$ means that the given digraph has an arc from $u$ to~$v$
or vice versa.
In particular, $F_1=\{v\mid 1\adj v\}$ is the set of undirected
...
(Well, we actually need to match directionally. A new arc must enter
a sink or leave a source. This code is more general than necessary,
because it was copied from {\mc DYNABIDIHAM}: It
actually checks for a {\it bidirectional\/} match.)
