...
\N[2 koda-ruskey.w]{1}{1}Intro. This program contains two implementations of
the Koda--Ruskey
...
\M[53 koda-ruskey.w]{2}In this step we parse the forest into an array of
``scopes'':
...
\M[79 koda-ruskey.w]{3}\B\X3:Global variables\X${}\E{}$\6
...
\N[84 koda-ruskey.w]{1}{4}The coroutine implementation. Our first
...
\M[141 koda-ruskey.w]{5}When coroutine \PB{\|p} calls coroutine \PB{\|q}, it
sets \PB{$\|p\MG\\{state}$} to an
...
\M[172 koda-ruskey.w]{6}In its initial state~0, a coroutine turns its bit on,
returns~\PB{\\{true}},
and enters state~1.
...
\M[178 koda-ruskey.w]{7}The purpose of state 1 is to run through all bit
patterns of the
...
\M[196 koda-ruskey.w]{8}State 4 is rather like state 1, except that the child
coroutine is now
...
\M[212 koda-ruskey.w]{9}Hey, the implementation is done already, except that we
have to
...
\M[222 koda-ruskey.w]{10}We allocate a special cnode to represent the external
...
\M[237 koda-ruskey.w]{11}\B\X3:Global variables\X${}\mathrel+\E{}$\6
\&{cnode} ${}\\{cnode\_table}[\\{forestsize}+\T{1}]{}$;\C{ the cnodes }\6
\&{boolean} \\{coresult};\C{ value returned by a coroutine }\par
\fi

\M[241 koda-ruskey.w]{12}States 8 and greater are reserved for the external
...
\M[259 koda-ruskey.w]{13}\B\X13:Print out all the current cnode bits\X${}\E{}$\6
\&{for} ${}(\|k\K\T{0};{}$ ${}\|k<\\{nn};{}$ ${}\|k\PP){}$\1\5
${}\\{putchar}(\.{'0'}+\\{cnode\_table}[\|k].\\{bit});{}$\2\6
\\{putchar}(\.{'\\n'});\par
\U12.\fi

\N[263 koda-ruskey.w]{1}{14}The loopless implementation. Our coroutine
...
\M[318 koda-ruskey.w]{15}Here now is the basic outline of the loopless
implementation:
...
\M[345 koda-ruskey.w]{16}Initialization of the lnodes is similar to
...
\M[364 koda-ruskey.w]{17}\B\X3:Global variables\X${}\mathrel+\E{}$\6
\&{lnode} ${}\\{lnode\_table}[\\{forestsize}+\T{1}]{}$;\C{ the lnodes }\6
\&{boolean} \\{been\_there\_and\_done\_that};\par
\fi

\M[368 koda-ruskey.w]{18}\B\X18:Set \PB{\|p} to the rightmost active node of
...
\M[373 koda-ruskey.w]{19}\B\X19:Insert the children of \PB{\|p} after \PB{\|p}
in the fringe\X${}\E{}$\6
\&{if} ${}(\|p\MG\\{lchild}){}$\5
${}\{{}$\1\6
${}\|q\K\|p\MG\\{right};{}$\6
...
\M[380 koda-ruskey.w]{20}\B\X20:Delete the children of \PB{\|p} from the fringe%
\X${}\E{}$\6
\&{if} ${}(\|p\MG\\{lchild}){}$\5
${}\{{}$\1\6
...
\M[386 koda-ruskey.w]{21}At this point we know that \PB{$\|p\MG\\{right}$} is
active.
...
\M[392 koda-ruskey.w]{22}\B\X22:Print out all the current lnode bits\X${}\E{}$\6
\&{for} ${}(\|k\K\T{0};{}$ ${}\|k<\\{nn};{}$ ${}\|k\PP){}$\1\5
${}\\{putchar}(\.{'0'}+\\{lnode\_table}[\|k].\\{bit});{}$\2\6
\\{putchar}(\.{'\\n'});\par
\U15.\fi

\M[396 koda-ruskey.w]{23}I used the following code when debugging.
...
\N[407 koda-ruskey.w]{1}{24}Index.
