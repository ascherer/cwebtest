...
closed knight's tours of $8\times5$, $8\times6$, \dots, $8\times20$ boards,
...
Instead of naming its vertices 1 thru~12, let's use the cell names
00, 10, 20, 30, 01, 02, \dots, 22, 32.
There are nine 4-configs, obtained from the
fourteen edges of $G$ by first omitting the
edges $01\adj22$, $02\adj21$, $11\adj32$, and $12\adj31$
(which don't involve the first four vertices $\{00,10,20,30\}$),
then omitting one of the edges $\{02\adj10,\ 10\adj22,\ 10\adj31\}$
(which touch~10), and then omitting one of the edges
$\{01\adj20,\ 12\adj20,\ 20\adj32\}$ (which touch~20). If, say, we
remove $10\adj31$ and $20\adj32$, there are $t=2$ subpaths:
$01\adj20\adj12\adj00\adj21$ and
$02\adj10\adj22\adj30\adj11$. Therefore the codes for cells
(01, 11, 21, 31, 02, 12, 22, 32) of the frontier are
respectively (1, 2, 1, 0, 2, $-1$, $-1$, 0).
...
The class represented by (1, 2, 1, 0, 2, $-1$, $-1$, 0) actually turns out
to have no successors in this example,
because cell 12 is an inner vertex; that makes edge
$01\adj12$ unavailable. (Think about it.)

@ [{\it Historical notes:}\enspace
Computations of $3\times n$ knight's tours when $n$ is arbitrarily large
were first carried out in 1994, independently
by Noam Elkies and D.~E. Knuth, who learned of each other's work during
a chance encounter in Berkeley. (See Knuth's {\sl Selected Papers on
Fun and Games\/} (2011), Chapter~42; also OEIS A070030.)
Euler had proved in 1759 that there are no closed tours on a $4\times n$ board.
Johan de Ruiter realized that $m\times n$ boards for $m>4$ were computationally
feasible too;
he obtained the results for $m=5$ and $m=6$ in 2010 (see OEIS A175855,
A175881).
%In the following year `KeyTo9\_$\mskip2mu$Fans' and Zhao Hui Du
In the following year Yi Yang and Zhao Hui Du
extended the calculations to
$m=7$ and $m=8$ (see OEIS A193054, A193055). The number of closed knight's tours
on a normal chessboard ($m=n=8$) was a centuries-old problem, and
for many years Knuth mistakenly believed that it would be too difficult for
a computer to count them in a reasonable time.
It was first solved in 1997 by Brendan McKay (see OEIS A001230),
who found that the exact number is 13,267,364,410,532,
of which 1,658,420,855,433 are essentially distinct.]

[Of course knight graphs are graphs with very special properties.
The author knows of no prior work that computes the cycle counts
for all of the initial induced subgraphs when $G$ is a completely general graph.
However, he learned after writing this that a somewhat similar algorithm
had been presented by Ville H. Pettersson, in
{\sl Electronic Journal of Combinatorics\/ \bf21} (2014), \#P4.7;
Pettersson's frontiers are ``dual'' to those
considered here. This program was inspired by
some of the ideas in Yang and Du's computation of knight counts, together
with the author's previous program {\mc DYNAKNIGHT}.]
...
@d memsize 3000000000LL /* max mem size; must be \&{memtyp}, sufficiently large */
@d wtsize  1000000000LL /* ditto for the max number of classes */
@d oldmemsize 450000000 /* see above; this is generous */
@d deg 9 /* the codes $a_l$ in keys must be less than this */
