...
superslitherlink, by modifying "dlx2-loop.ch" (which was written long ago).
...
@y
@ Each \&{item} struct contains seven fields:
The |name| is the user-specified identifier;
|next| and |prev| point to adjacent items, when this
item is part of a doubly linked list;
two fields |left| and |right| are explained below;
@z
@x
@<Type...@>=
