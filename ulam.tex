...
\N[2 ulam.w]{1}{1}Intro. I'm trying to calculate a few million Ulam numbers.
This sequence
...
\M[82 ulam.w]{2}As we compute, we'll implicitly have $k=32\PB{\\{kq}}+\PB{%
...
\M[102 ulam.w]{3}\B\X3:Set \PB{$(\\{ubit}[\\{jj}],\\{vbit}[\\{jj}])$} to \PB{$(%
\\{ubit}[\\{jj}],\\{vbit}[\\{jj}])$}${}\circ\PB{\\{del}}$\X${}\E{}$\6
$\|u\K(\\{ubit}[\\{jj}]\XOR\\{del})\AND\CM\\{vbit}[\\{jj}];{}$\6
${}\\{vbit}[\\{jj}]\MRL{{\OR}{\K}}\\{ubit}[\\{jj}]\AND\\{del};{}$\6
${}\\{ubit}[\\{jj}]\K\|u{}$;\par
\U2.\fi

\M[107 ulam.w]{4}\B\X4:Advance $k$ to $U_{n+1}$ and advance $n$\X${}\E{}$\6
...
\M[120 ulam.w]{5}\B\X5:Set up the \PB{\\{table}}\X${}\E{}$\6
\&{for} ${}(\|j\K\T{2};{}$ ${}\|j<\T{256};{}$ ${}\|j\MRL{{\LL}{\K}}\T{1}){}$\1\5
${}\\{table}[\|j]\K\T{1}+\\{table}[\|j\GG\T{1}]{}$;\2\par
\U1.\fi

\M[123 ulam.w]{6}\B\X6:Print gap stats\X${}\E{}$\6
