...
\N{1}{1}Intro. This program generates clauses for the transition relation
...
\M{2}So here's the overall outline of the program.
...
\M{6}Clauses are assembled in the \PB{\\{clause}} array (surprise), where we
...
\M{7}And here's another, which puts a type-0 literal into \PB{\\{clause}}.
...
\M{8}The \PB{\|d} and \PB{\|e} subroutines are called for only one-fourth
...
\M{9}The \PB{\|f} subroutine can't be shared quite so often. But we
...
\M{10}The \PB{\|g} subroutine cleans up the dregs, by somewhat tediously
...
\M{11}Fortunately the \PB{\|b} subroutine {\it can\/} be shared (since \PB{\|x}
is always
...
\M{12}The (unshared) \PB{\|c} subroutine handles the other four neighbors,
...
\M{13}Totals over all eight neighbors are then deduced by the \PB{\|a}
subroutine.
...
\M{14}Finally, as mentioned at the beginning, $z'$ is determined
