...
\N[2 achain0.w]{1}{1}Intro. This program is a transcription of the code that I
wrote in
...
\M[66 achain0.w]{2}\B\X2:Process the command line\X${}\E{}$\6
\&{if} ${}(\\{argc}\I\T{3}){}$\5
${}\{{}$\1\6
...
\M[82 achain0.w]{3}\B\X3:Output the value of $l(n)$\X${}\E{}$\6
$\\{fprintf}(\\{outfile},\39\.{"\%c"},\39\|l[\|n]+\.{'\ '});{}$\6
\\{fflush}(\\{outfile});\C{ make sure the result is viewable immediately }\par
\U1.\fi

\M[86 achain0.w]{4}At this point I compute the ``lower bound'' $\lfloor\lg n%
\rfloor+3$,
...
\M[97 achain0.w]{5}Three elementary and well-known upper bounds are considered:
...
\M[124 achain0.w]{6}It's important to try the factor method even when \PB{$\|j%
\Z\T{4}$}, because
...
\N[145 achain0.w]{1}{7}The interesting part. All the above was necessary just
to get
...
\M[162 achain0.w]{8}We maintain a stack of subproblems, as usual when
backtracking.
...
\M[235 achain0.w]{9}The heart of the computation is the following routine,
which decides
...
\M[273 achain0.w]{10}The other case is essentially the same. So if I have a bug
in
