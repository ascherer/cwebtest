...
\N[2 bdd15.w]{1}{1}Intro. This program is the fifteenth in a series of
...
\M[73 bdd15.w]{2}For simplicity, I do my own memory allocation in a big array
...
\M[100 bdd15.w]{3}Here's the overall program structure:
...
\M[123 bdd15.w]{4}\B\D$\\{file\_given}$ \5
$(\\{argc}\E\T{2}{}$)\par
...
\M[131 bdd15.w]{5}\B\X5:Global variables\X${}\E{}$\6
...
\M[135 bdd15.w]{6}\B\X6:Initialize everything\X${}\E{}$\6
\\{gb\_init\_rand}(\T{0});\C{ initialize the random number generator }\par
\As10, 13, 45\ETs63.
\U3.\fi

\M[138 bdd15.w]{7}One of the main things I hope to learn with this program is
the total
...
\M[166 bdd15.w]{8}\B\X8:Subroutines\X${}\E{}$\6
...
\M[174 bdd15.w]{9}This program uses `\PB{\&{long} \&{long}}' to refer to 64-bit
integers,
...
\M[186 bdd15.w]{10}\B\X6:Initialize everything\X${}\mathrel+\E{}$\6
...
\M[192 bdd15.w]{11}Speaking of compilers, the one I use at present insists that
...
\N[231 bdd15.w]{1}{12}Dynamic arrays. Before I get into the ZDD stuff, I might
as well
...
\M[271 bdd15.w]{13}Here's how we launch the dynamic memory setup.
...
\M[293 bdd15.w]{14}\B\X5:Global variables\X${}\mathrel+\E{}$\6
...
\M[304 bdd15.w]{15}Here's how we get a fresh (but uninitialized) node.
...
\M[334 bdd15.w]{16}Conversely, nodes can always be recycled. In such cases,
there
...
\M[344 bdd15.w]{17}Occupation and liberation of pages is similar, but it takes
place
...
\M[373 bdd15.w]{18}\B\X18:Print total memory usage\X${}\E{}$\6
...
\M[378 bdd15.w]{19}\B\X19:Local variables\X${}\E{}$\6
\&{register} \&{int} \|j${},{}$ \|k;\par
\A113.
\U3.\fi

\N[381 bdd15.w]{1}{20}Variables and hash tables. Our ZDD base represents
functions
...
\M[427 bdd15.w]{21}Every node \PB{\|p} that branches on $x_v$ in the ZDD has a
field \PB{$\|p\MG\\{index}$},
...
\M[445 bdd15.w]{22}Variable $x_v$ in this documentation means the variable
whose information
...
\M[459 bdd15.w]{23}Before any variables are used, we call the \PB{%
\\{createvars}} routine
...
\M[470 bdd15.w]{24}We need a node at each level that means ``tautology from
here on,''
...
\M[508 bdd15.w]{25}The simplest nonconstant Boolean expression is a projection
function, $x_v$.
...
\M[528 bdd15.w]{26}I sometimes like to use subroutines before I'm in the mood
to write
...
\M[536 bdd15.w]{27}Now, however, I'm ready to tackle the \PB{\\{unique\_find}}
subroutine,
...
\M[601 bdd15.w]{28}\B\X26:Templates for subroutines\X${}\mathrel+\E{}$\6
...
\M[606 bdd15.w]{29}Before we can call \PB{\\{unique\_find}}, we need a hash
table to work with.
We start small.
...
\M[616 bdd15.w]{30}A little timer starts ticking at the beginning of this
program,
...
\M[644 bdd15.w]{31}\B\X5:Global variables\X${}\mathrel+\E{}$\6
\&{unsigned} \&{long} \&{long} \\{timer};\par
\fi

\M[647 bdd15.w]{32}Brand-new nodes enter the fray here.
...
\M[664 bdd15.w]{33}We get to this part of the code when the table has become
too dense.
...
\M[685 bdd15.w]{34}\B\D$\\{maxmask}$ \5
...
\M[709 bdd15.w]{35}Some subtle cases can arise at this point.
...
\M[736 bdd15.w]{36}While I've got linear probing firmly in mind, I might as
well
...
\M[760 bdd15.w]{37}Deletion from a linearly probed hash table is tricky, as
noted in
...
\M[783 bdd15.w]{38}At least one node, \PB{$\|v\MG\\{elt}$}, branches on $x_v$
at this point.
...
\M[799 bdd15.w]{39}Finally, another algorithm learned from Rudell. To prove its
correctness,
...
\N[818 bdd15.w]{1}{40}The cache. The other principal data structure we need,
...
\M[877 bdd15.w]{41}The cache always occupies $2^e$ pages of the dynamic memory,
...
\M[898 bdd15.w]{42}The following subroutines, useful for debugging, print out
the
...
\M[922 bdd15.w]{43}Many of the symbolic names here are presently unused. I've
filled them
...
\M[933 bdd15.w]{44}The threshold is set to half the total number of cache
slots,
...
\M[973 bdd15.w]{45}\B\X6:Initialize everything\X${}\mathrel+\E{}$\6
\\{cache\_init}(\,);\par
\fi

\M[976 bdd15.w]{46}Here's how we look for a memo in the cache. Memos might
point to dead
...
\M[1012 bdd15.w]{47}Insertion into the cache is even easier, except that we
might
...
\M[1034 bdd15.w]{48}\B\X48:Double the cache size\X${}\E{}$\6
...
\M[1056 bdd15.w]{49}\B\X49:Recache the items in the bottom half\X${}\E{}$\6
...
\M[1069 bdd15.w]{50}Before we purge elements from the unique tables, we need to
purge all
...
\M[1092 bdd15.w]{51}\B\X51:Downsize the cache if it has now become too sparse%
\X${}\E{}$\6
...
\N[1114 bdd15.w]{1}{52}ZDD structure. The reader of this program ought to be
familiar with
...
\M[1131 bdd15.w]{53}Sometimes we want to mark the nodes of a subfunction
temporarily.
...
\M[1146 bdd15.w]{54}We need to remove those marks soon after \PB{\\{mark}} has
been called,
...
\M[1160 bdd15.w]{55}Here's a simple routine that prints out the current ZDDs,
in order
...
\M[1194 bdd15.w]{56}The masking feature is useful when we want to print out
only a single ZDD.
...
\M[1209 bdd15.w]{57}\B\X8:Subroutines\X${}\mathrel+\E{}$\6
...
\M[1228 bdd15.w]{58}\B\X58:Print the number of marked nodes that branch on \PB{%
\|v}\X${}\E{}$\6
...
\M[1239 bdd15.w]{59}In order to deal efficiently with large ZDDs, we've
introduced highly
...
\M[1280 bdd15.w]{60}Sanity checking is done with a ``shadow memory'' \PB{%
\\{smem}}, which is just
...
\M[1300 bdd15.w]{61}\B\D$\\{complain}(\\{complaint})$ \6
${}\{{}$\5
...
\M[1338 bdd15.w]{62}The macros above and the \PB{\\{who\_points\_to}} routine
below rely on the fact that
\PB{$\&{sizeof}(\&{node})\K\T{16}$}.

\fi

\M[1341 bdd15.w]{63}\B\X6:Initialize everything\X${}\mathrel+\E{}$\6
...
\M[1347 bdd15.w]{64}\B\X64:Check that \PB{\|p} is findable in the unique table%
\X${}\E{}$\6
${}\{{}$\1\6
\&{register} \&{addr} ${}{*}\\{hash};{}$\6
...
\M[1364 bdd15.w]{65}\B\X65:Check the list of free nodes\X${}\E{}$\6
...
\M[1372 bdd15.w]{66}\B\X66:Compute the ghost index fields\X${}\E{}$\6
...
\M[1394 bdd15.w]{67}\B\X67:Check the reference counts\X${}\E{}$\6
...
\M[1406 bdd15.w]{68}If a reference count turns out to be wrong, I'll probably
want to know why.
...
\M[1420 bdd15.w]{69}We've seen that every superlegimate node is findable in the
...
\M[1452 bdd15.w]{70}The fields in cache memos that refer to nodes should refer
to
legitimate nodes.
...
\M[1474 bdd15.w]{71}Finally, \PB{\\{sanity\_check}} ensures that we haven't
...
\M[1491 bdd15.w]{72}The following routine brings a dead node back to life.
...
\M[1510 bdd15.w]{73}Conversely, we sometimes must go the other way, with as
much dignity
as we can muster.
...
\N[1530 bdd15.w]{1}{74}Binary operations. OK, now we've got a bunch of powerful
routines for making
...
\M[1573 bdd15.w]{75}I assume that \PB{$\|f\MG\\{lo}$} and \PB{$\|f\MG\\{hi}$}
...
\M[1596 bdd15.w]{76}With ZDDs, $f\lor g$ is {\it not\/} dual to $f\land g$, as
it
was in {\mc BDD14}.
...
\M[1611 bdd15.w]{77}\B\X77:Find $f\lor g$ recursively\X${}\E{}$\6
$\\{rmems}\PP{}$;\C{ track recursion overhead }\6
${}\\{vf}\K\\{thevar}(\|f);{}$\6
${}\\{vg}\K\\{thevar}(\|g);{}$\6
\&{if} ${}(\\{vf}<\\{vg}){}$\5
...
\M[1646 bdd15.w]{78}Exclusive or is much the same.
...
\M[1660 bdd15.w]{79}\B\X79:Find $f\oplus g$ recursively\X${}\E{}$\6
$\\{rmems}\PP{}$;\C{ track recursion overhead }\6
${}\\{vf}\K\\{thevar}(\|f);{}$\6
${}\\{vg}\K\\{thevar}(\|g);{}$\6
\&{if} ${}(\\{vf}<\\{vg}){}$\5
...
\M[1693 bdd15.w]{80}ZDDs work well only with ``normal'' operators $\circ$,
namely operators
...
\M[1715 bdd15.w]{81}\B\X81:Find $f\land\bar g$ recursively\X${}\E{}$\6
$\\{rmems}\PP{}$;\C{ track recursion overhead }\6
\&{if} ${}(\\{vf}<\\{vg}){}$\5
${}\{{}$\1\6
...
\M[1738 bdd15.w]{82}The product operation $f\sqcup g$ is new in {\mc BDD15}: It
corresponds to
...
\M[1766 bdd15.w]{83}In this step I compute $g_l\lor g_h$ and join it with
$f_h$,
...
\M[1816 bdd15.w]{84}The disproduct operation is similar to product, but it
evaluates
...
\M[1843 bdd15.w]{85}\B\X85:Find the disjoint $f\sqcup g$ recursively\X${}\E{}$\6
$\\{rmems}\PP{}$;\C{ track recursion overhead }\6
\&{if} ${}(\\{vf}\I\\{vg}){}$\5
${}\{{}$\1\6
...
\M[1876 bdd15.w]{86}The coproduct operation $f\sqcap g$, which is analogous to
$f\sqcup g$,
...
\M[1897 bdd15.w]{87}\B\X87:Find $f\sqcap g$ recursively\X${}\E{}$\6
$\\{rmems}\PP{}$;\C{ track recursion overhead }\6
...
\M[1932 bdd15.w]{88}Similarly, there's a delta operation $f\bindel g
...
\M[1956 bdd15.w]{89}\B\X89:Find $f\bindel g$ recursively\X${}\E{}$\6
$\\{rmems}\PP{}$;\C{ track recursion overhead }\6
\&{if} ${}(\\{vf}\I\\{vg}){}$\5
${}\{{}$\1\6
...
\M[1998 bdd15.w]{90}The quotient and remainder operations have a somewhat
different
...
\M[2028 bdd15.w]{91}\B\X91:Find $f\bmod g$ recursively\X${}\E{}$\6
$\\{rmems}\PP;{}$\6
...
\M[2044 bdd15.w]{92}\B\X8:Subroutines\X${}\mathrel+\E{}$\6
...
\M[2059 bdd15.w]{93}\B\X93:Find $f/g$ recursively in the easy case\X${}\E{}$\6
$\\{rmems}\PP;{}$\6
...
\M[2075 bdd15.w]{94}Now for the general case of division, which also simplifies
in several
...
\M[2096 bdd15.w]{95}\B\X95:Find $f/g$ recursively in the general case\X${}\E{}$%
\6
$\\{rmems}\PP;{}$\6
${}\|o,\39\\{vg}\K\\{thevar}(\|g);{}$\6
...
\M[2123 bdd15.w]{96}At present, I don't look for any special cases of the
remainder operation
...
\N[2154 bdd15.w]{1}{97}Ternary operations. All operations can be reduced to
binary operations,
...
\M[2199 bdd15.w]{98}\B\X98:Find $(f{?}\ g{:}\ h)$ recursively\X${}\E{}$\6
$\\{rmems}\PP{}$;\C{ track recursion overhead }\6
\&{if} ${}(\|v<\\{vf}){}$\5
...
\M[2225 bdd15.w]{99}The median (or majority) operation $\langle fgh\rangle$ has
lots of nice
symmetry.
...
\M[2250 bdd15.w]{100}\B\X100:Find $\langle fgh\rangle$ recursively\X${}\E{}$\6
$\\{rmems}\PP{}$;\C{ track recursion overhead }\6
...
\M[2269 bdd15.w]{101}More symmetry here.
...
\M[2311 bdd15.w]{102}\B\X102:Find $f\land g\land h$ recursively\X${}\E{}$\6
$\\{rmems}\PP{}$;\C{ track recursion overhead }\6
...
\M[2328 bdd15.w]{103}The \PB{\\{symfunc}} operation is a ternary relation of a
different kind:
...
\M[2383 bdd15.w]{104}There's also a kludgy ternary operation intended for
building arbitrary
...
\N[2408 bdd15.w]{1}{105}Top-level calls. As mentioned above, there's a
...
\M[2447 bdd15.w]{106}\B\X26:Templates for subroutines\X${}\mathrel+\E{}$\6
\&{void} \\{attempt\_repairs}(\&{void});\C{ collect garbage or something if
there's hope }\par
\fi

\M[2450 bdd15.w]{107}\B\X8:Subroutines\X${}\mathrel+\E{}$\6
...
\N[2497 bdd15.w]{1}{108}Parsing the commands.
...
\M[2510 bdd15.w]{109}Before we do any commands, it's helpful to ensure that no
embarrassing
...
\M[2528 bdd15.w]{111}\B\X111:Fill \PB{\\{buf}} with the next command, or \PB{%
\&{goto} \\{alldone}}\X${}\E{}$\6
\&{if} (\\{infile})\5
${}\{{}$\1\6
...
\M[2544 bdd15.w]{112}The first nonblank character of each line identifies the
type of command.
...
\M[2591 bdd15.w]{113}\B\X19:Local variables\X${}\mathrel+\E{}$\6
...
\M[2598 bdd15.w]{114}The \<special> command \.{include} \<filename> starts up a
new infile.
...
\M[2616 bdd15.w]{115}The command `\.{p3}' prints out the ZDD for $f_3$; the
command
...
\M[2635 bdd15.w]{116}\B\X116:Output a function\X${}\E{}$\6
\\{getkf};\6
...
\M[2642 bdd15.w]{117}\B\X117:Print the current variable ordering\X${}\E{}$\6
\&{for} ${}(\|v\K\\{varhead};{}$ ${}\|v<\\{topofvars};{}$ ${}\|v\PP){}$\1\5
${}\\{printf}(\.{"\ x\%d"},\39\|v\MG\\{name});{}$\2\6
\\{printf}(\.{"\\n"});\par
\U112.\fi

\M[2646 bdd15.w]{118}My little finite-state automaton.
...
\M[2659 bdd15.w]{119}\B\D$\\{checknull}(\|p)$ \6
\&{if} ${}(\R\|p){}$\5
${}\{{}$\5
...
\M[2675 bdd15.w]{120}The user shouldn't access any constants until specifying
the
...
\M[2691 bdd15.w]{121}Many of the operations implemented in {\mc BDD14} are not
present
(yet?) in {\mc BDD15}.
...
\M[2718 bdd15.w]{122}\B\X122:Get the second operand, \PB{\|q}\X${}\E{}$\6
\\{passblanks};\6
\&{switch} ${}({*}\|c\PP){}$\5
${}\{{}$\1\6
\4\&{case} \.{'e'}:\5
...
\M[2728 bdd15.w]{123}\B\X123:If the operator is ternary, get the third operand,
\PB{\|r}\X${}\E{}$\6
\\{passblanks};\6
...
\M[2745 bdd15.w]{124}We have made sure that all the necessary operands are non-%
\PB{$\NULL$}.
...
\M[2756 bdd15.w]{125}The \PB{\\{sanity\_check}} routine tells me that
...
\M[2764 bdd15.w]{126}\B\X126:Dereference the left-hand side\X${}\E{}$\6
\&{if} ${}(\|o,\39\|f[\\{lhs}]){}$\5
${}\{{}$\1\6
\\{deref}(\|f[\\{lhs}]);\6
${}\|o,\39\|f[\\{lhs}]\K\NULL;{}$\6
...
\M[2770 bdd15.w]{127}In a long calculation, it's nice to get progress reports
by setting
...
\N[2778 bdd15.w]{1}{128}Reordering. All of the algorithms for changing the
order of variables
in a ZDD base are
...
\M[2798 bdd15.w]{129}Before we diddle with such a sensitive thing as the order
of branching,
...
\M[2834 bdd15.w]{130}\B\X5:Global variables\X${}\mathrel+\E{}$\6
...
\M[2839 bdd15.w]{131}We classify the nodes on levels $u$ and $v$ into four
categories:
...
\M[2908 bdd15.w]{132}Here's a cute algorithm something like the inner loop of
quicksort.
...
\M[2945 bdd15.w]{133}We temporarily save the pages of the old unique table,
since they
...
\M[2970 bdd15.w]{134}\B\X5:Global variables\X${}\mathrel+\E{}$\6
\&{addr} \\{savebase}[\\{maxhashpages}];\C{ pages to be discarded after
swapping }\par
\fi

\M[2973 bdd15.w]{135}The \PB{\\{new\_unique}} routine inaugurates an empty
...
\M[2997 bdd15.w]{136}The \PB{\\{insert\_node}} subroutine is somewhat analogous
...
\M[3024 bdd15.w]{137}\B\X137:Create a new unique table for $x_v$ and move the
...
\M[3033 bdd15.w]{138}The most dramatic change caused by swapping occurs in this
step.
...
\M[3054 bdd15.w]{139}The \PB{\\{swap\_find}} procedure in the
...
\M[3077 bdd15.w]{140}\B\X140:Create a newbie and return it\X${}\E{}$\6
\&{if} ${}(\MM\\{free}\Z\\{mask}\GG\T{4}){}$\1\5
\X33:Double the table size and \PB{\&{goto} \\{restart}}\X;\2\6
${}\|p\K\\{reserve\_node}(\,);{}$\6
${}\\{storenode}(\|v,\39\|k,\39\|p){}$;\5
...
\M[3088 bdd15.w]{141}\B\X141:Delete the lists of solitary, tangled, and hidden
nodes\X${}\E{}$\6
\&{for} ${}(\|k\K\T{0};{}$ ${}\|k\Z\\{umask}\GG\\{logpagesize};{}$ ${}\|k%
\PP){}$\1\5
...
\M[3096 bdd15.w]{142}All \PB{\\{elt}} and \PB{\\{taut}} functions are kept
...
\M[3118 bdd15.w]{143}The \PB{\\{swap}} subroutine is now complete. I can safely
declare it,
...
\M[3124 bdd15.w]{144}\B\X144:Bubble sort to reestablish the natural variable
order\X${}\E{}$\6
\&{if} (\\{totalvars})\5
${}\{{}$\1\6
\\{reorder\_init}(\,);\C{ prepare for reordering }\6
...
\M[3138 bdd15.w]{145}Now we come to the \PB{\\{sift}} routine, which finds the
best position
...
\M[3150 bdd15.w]{146}At this point \PB{$\|v\MG\\{aux}$} is the position of \PB{%
\|v} among all
...
\M[3174 bdd15.w]{147}In a production version of this program, I would stop
sifting
...
\M[3205 bdd15.w]{148}\B\X148:Explore in the downward direction\X${}\E{}$\6
...
\M[3230 bdd15.w]{149}The \PB{\\{siftall}} subroutine sifts until every variable
has found
...
\M[3255 bdd15.w]{150}Sifting is invoked automatically when the number of nodes
is
...
\M[3267 bdd15.w]{151}\B\X151:Invoke autosifting\X${}\E{}$\6
${}\{{}$\1\6
...
\M[3277 bdd15.w]{152}\B\X5:Global variables\X${}\mathrel+\E{}$\6
...
\N[3281 bdd15.w]{1}{153}Triage and housekeeping.
...
\M[3308 bdd15.w]{154}The global variable \PB{\\{last\_ditch}} is set nonzero
when we resort to
...
\M[3315 bdd15.w]{155}\B\X5:Global variables\X${}\mathrel+\E{}$\6
\&{int} \\{last\_ditch};\C{ are we backed up against the wall? }\par
\fi

\M[3318 bdd15.w]{156}\B\X8:Subroutines\X${}\mathrel+\E{}$\6
...
\N[3332 bdd15.w]{1}{157}Mathematica output. An afterthought: It's easy to
output a
...
\M[3341 bdd15.w]{158}\B\X5:Global variables\X${}\mathrel+\E{}$\6
\&{FILE} ${}{*}\\{outfile};{}$\6
\&{int} \\{outcount};\C{ the number of files output so far }\par
\fi

\M[3345 bdd15.w]{159}\B\X8:Subroutines\X${}\mathrel+\E{}$\6
...
\M[3369 bdd15.w]{160}\B\X160:Generate Mathematica outputs for variable \PB{\|v}%
\X${}\E{}$\6
${}\{{}$\1\6
\&{for} ${}(\|k\K\T{0};{}$ ${}\|k<\|v\MG\\{mask};{}$ ${}\|k\MRL{+{\K}}%
\&{sizeof}(\&{addr})){}$\5
...
\M[3379 bdd15.w]{161}\B\X161:Generate a Mathematica line for node \PB{\|q}\X${}%
\E{}$\6
