...
\N{1}{1}Introduction. The ur-game for computers --- Adventure --- was
originally
...
\M{2}To run the program with, say, a {\mc UNIX} shell, just type `\.{advent}'
...
\M{3}The subroutines of this program are declared first with a prototype,
...
\N{1}{4}The vocabulary. Throughout the remainder of this documentation, ``you''
are
...
\M{5}When you type a word, we first convert uppercase letters to lowercase;
then
...
\M{6}Here is the subroutine that puts words into our vocabulary, when the
...
\M{8}While we're at it, let's write the program that will look up a word.
...
\M{9}The \PB{\&{motion}} words specify either a direction or a simple action or
a place.
...
\M{10}And here is how they enter our vocabulary.
...
\M{11}The \PB{\&{object}} words refer to things like a lamp, a bird, batteries,
etc.;
...
\M{12}Most of the objects correspond to words in our vocabulary.
...
\M{13}The \PB{\&{action}} words tell us to do something that's usually
nontrivial.
...
\M{14}Many of the action words have several synonyms.
...
\M{15}\B\X7:Global variables\X${}\mathrel+\E{}$\6
\&{char} ${}{*}\\{default\_msg}[\T{30}]{}$;\C{ messages for untoward actions,
if nonzero }\par
\fi

\M{16}Finally, our vocabulary is rounded out by words like \.{help}, which
...
\M{17}\B\X7:Global variables\X${}\mathrel+\E{}$\6
\&{char} ${}{*}\\{message}[\T{13}]{}$;\C{ messages tied to certain vocabulary
words }\par
\fi

\N{1}{18}Cave data. You might be in any of more than 100 places as you wander
...
\M{19}Speaking of program logic, the complex cave dynamics are essentially
...
\M{20}Suppose you're at location $l$. Then \PB{\\{start}[\|l]} is the first
...
\N{1}{21}Cave connections. Now we are ready to build the fundamental table
...
\M{22}\B\X22:Additional local registers\X${}\E{}$\6
\&{register} \&{instruction} ${}{*}\|q,{}$ ${}{*}\\{qq}{}$;\par
\As68\ET144.
\U2.\fi

\M{23}The \PB{\\{road}} is where you start; its \PB{\\{long\_desc}} is now
famous,
...
\M{24}There's nothing up the hill, but a good explorer has to try anyway.
...
\M{25}The house initially contains several objects: keys, food, a bottle, and
...
\M{26}A foolish consistency is the hobgoblin of little minds. (Emerson)
...
\M{27}The instructions here keep you in the \PB{\\{forest}} with probability 50%
\%,
...
\M{28}You're getting closer. (But the program has forgotten that
...
\M{29}We'll see later that the \PB{\.{GRATE}} will change from state 0 to state
1 if you
...
\M{30}If you've come this far, you're probably hooked, although your adventure
has barely begun.
...
\M{31}Go West, young man. (If you've got a lamp.)
...
\M{32}Welcome to the main caverns and a deeper level of adventures.
...
\M{33}To the left or south of the misty threshold, you might spot the first
treasure.
...
\M{34}Unless you take a circuitous route to the other side of the Hall of
Mists,
...
\M{35}What you see here isn't exactly what you get; \.N takes you east and
...
\M{36}The twisty little passages of this maze are said to be all alike,
...
\M{37}\B\X23:Build the travel table\X${}\mathrel+\E{}$\6
$\\{make\_loc}(\\{brink},{}$\6
...
\M{38}Crawling west from \PB{\\{wmist}} instead of south, you encounter this.
...
\M{39}Recall that the `100' on the last instruction above means, ``Dwarves
...
\M{40}Going north from the long hall, we come to the vicinity of another large
...
\M{41}North of the mountain king's domain is a curious shuttle station
called~Y2,
...
\M{42}Next let's consider the east/west passage below \PB{\\{ns}}.
...
\M{43}A more-or-less self-contained cavelet can be found north of the complex
...
\M{44}A dangerous section lies east of the complex junction.
...
\M{45}Will Crowther, who actively explored and mapped many caves in Kentucky
...
\M{46}West of the quilt and the cheese is a room with two pits.
...
\M{47}Oho, you climbed the plant in the west pit! Now you're in another
...
\M{48}Meanwhile let's backtrack to another part of the cave possibly reachable
from Bedquilt.
...
\M{49}The described view from the west window, \PB{\\{window}},
...
\M{50}More treasures await you via the \PB{\\{low}} corridor.
...
\M{52}We forgot to mention the circuitous passage leading west from the
...
\M{54}Below \PB{\\{secret}} there's another way to reach the cheese.
...
\M{55}If you aren't having fun yet, wait till you meet the troll. The only way
to
...
\M{56}The only things not yet explored on this side of the troll bridge are
...
\M{57}A whole nuther cave with nine sites and additional treasures
...
\M{58}The two storage locations are accessible only from
...
\M{59}When the current location is \PB{\\{crack}} or higher, it's a
pseudo-location.
...
\M{60}Here are some forced actions that are less pleasant.
\Y\B\4\X23:Build the travel table\X${}\mathrel+\E{}$\6
$\\{make\_loc}(\\{neck},{}$\6
...
\M{61}The rest are more-or-less routine, except for \PB{\\{check}}---which
executes
...
\M{62}The table of instructions ends here; the remaining ``locations''
...
\N{1}{63}Data structures for objects. A fixed universe of objects was
...
\M{64}Here then is a simple subroutine to place an object at a given
...
\M{65}Similarly, we need a subroutine to pick up an object.
...
\M{66}The \PB{\\{is\_at\_loc}} subroutine tests if a possibly multipart object
is
...
\M{67}A few macros make it easy to get each object started.
...
\M{68}\B\X22:Additional local registers\X${}\mathrel+\E{}$\6
\&{register} \&{object} \|t;\par
\fi

\N{1}{69}Object data. Now it's time to build the object structures just
defined.
...
\M{70}And here are the one-place objects, some of which are immovable
...
\N{1}{71}Low-level input. Sometimes we need to ask you a question, for which
...
\M{72}The only other kind of input is almost as simple. You are supposed to
...
\M{73}A 20-character buffer would probably be big enough, but what the heck.
...
\N{1}{74}The main control loop. Now we've got enough low-level mechanisms in
...
\M{75}Here is our overall strategy for administering the game. It is understood
...
\M{76}Our main task in the simulation loop is to parse your input.
...
\M{77}\B\X7:Global variables\X${}\mathrel+\E{}$\6
...
\M{78}The \PB{\\{try\_motion}} macro is often used to end a major cycle.
...
\M{79}Here is the multiway branch where many kinds of actions can be launched.
...
\M{80}Here's a freely offered hint that may save you typing.
...
\M{81}\B\X7:Global variables\X${}\mathrel+\E{}$\6
\&{int} \\{west\_count};\C{ how many times have we parsed the word `\.{west}'?
}\par
\fi

\M{82}Maybe you said `\.{say}' and we said `\.{Say} \.{what?}' and you replied
...
\M{83}The verb `\.{enter}' is listed in our vocabulary as a motion rather than
...
\M{84}Cavers can become cadavers if they don't have light. We keep a variable
...
\M{85}\B\X85:Make special adjustments before looking at new input\X${}\E{}$\6
$\\{was\_dark}\K\\{dark}{}$;\par
\As158, 169\ETs182.
\U76.\fi

\M{86}After moving to \PB{\\{newloc}}, we act as your eyes.
...
\M{87}\B\X7:Global variables\X${}\mathrel+\E{}$\6
...
\M{88}If \PB{\.{TREADS}} are present but you have a heavy load, we don't
describe them.
...
\M{89}\B\X7:Global variables\X${}\mathrel+\E{}$\6
\&{int} \\{tally}${}\K\T{15}{}$;\C{ treasures awaiting you }\6
\&{int} \\{lost\_treasures};\C{ treasures that you won't find }\par
\fi

\M{90}When you specify an object, it must be at the current location, unless
the
...
\M{91}Henning Makholm has pointed out that the logic here makes \PB{\.{GRATE}}
a motion
...
\N{1}{92}Simple verbs. Let's get experience implementing the actions by
...
\M{93}Only the objects \PB{\.{GRATE}}, \PB{\.{DOOR}}, \PB{$\.{CLAM}/%
\.{OYSTER}$}, and \PB{\.{CHAIN}} can
...
\M{94}A request for an inventory is pretty simple too.
...
\M{95}Here are other requests about the mechanics of the game.
...
\M{96}\B\X7:Global variables\X${}\mathrel+\E{}$\6
\&{boolean} \\{gave\_up};\C{ did you quit while you were alive? }\par
\fi

\M{97}The \PB{\.{SAY}} routine is just an echo unless you say a magic word.
...
\M{99}Waving to the shadowy figure has no effect; but you might
...
\M{100}If asked to find an object that isn't visible, we give a caveat.
...
\M{101}Breaking and/or waking have no effect until the cave is closed,
...
\M{102}Here we deal with lighting or extinguishing the lamp. The variable
...
\M{103}\B\X7:Global variables\X${}\mathrel+\E{}$\6
\&{int} \\{limit};\C{ countdown till darkness }\par
\fi

\N{1}{104}Liquid assets. Readers of this program will already have noticed that
...
\M{105}Sometimes `\.{water}' and `\.{oil}' are used as verbs.
...
\M{106}If you ask simply to drink, we assume that you want water. If there's
...
\M{107}Pouring involves liquid from the bottle.
...
\M{108}\B\X108:Try to water the plant\X${}\E{}$\6
${}\{{}$\1\6
\&{if} ${}(\\{obj}\I\.{WATER}){}$\1\5
...
\M{109}\B\X109:Pour water or oil on the door\X${}\E{}$\6
\&{switch} (\\{obj})\5
${}\{{}$\1\6
\4\&{case} \.{WATER}:\5
${}\\{prop}[\.{DOOR}]\K\T{0};{}$\6
...
\M{110}You can fill the bottle only when it's empty and liquid is available.
...
\M{111}Filling the vase is a nasty business.
...
\M{112}Picking up a liquid depends, of course, on the status of the bottle.
...
\M{113}\B\X113:Check special cases for taking a liquid\X${}\E{}$\6
\&{if} ${}(\\{here}(\.{BOTTLE})\W\\{object\_in\_bottle}){}$\1\5
${}\\{obj}\K\.{BOTTLE};{}$\2\6
\&{else}\5
${}\{{}$\1\6
...
\M{114}\B\X114:Check special cases for taking a bird\X${}\E{}$\6
${}\{{}$\1\6
\&{if} (\\{toting}(\.{ROD}))\1\5
...
\M{115}Similarly, when dropping the bottle we must drop also its liquid
contents,
if any.
...
\N{1}{116}The other actions. Now that we understand how to write action
routines,
we're ready to complete the set.

\fi

\M{117}Dropping an object has special cases for the bird (which might attack
...
\M{118}\B\X118:Put coins in the vending machine\X${}\E{}$\6
${}\{{}$\1\6
\\{destroy}(\.{COINS});\6
...
\M{119}\PB{\.{TROLL2}} is the absent troll. We move the troll bridge up to
first in
...
\M{120}\B\X120:Check special cases for dropping the bird\X${}\E{}$\6
${}\{{}$\1\6
\&{if} (\\{here}(\.{SNAKE}))\5
${}\{{}$\1\6
...
\M{121}\B\X121:Check special cases for dropping the vase\X${}\E{}$\6
${}\{{}$\1\6
...
\M{122}Throwing is like dropping, except that it covers a few more cases.
...
\M{123}This'll teach you a lesson.
...
\M{124}If you toss the vase, the skillful troll will catch it before it breaks.
...
\M{125}When you try to attack, the action becomes violent.
...
\M{126}Attackable objects fall into two categories: enemies (snake, dwarf,
etc.)\
...
\M{127}\B\X127:Dispatch the poor bird\X${}\E{}$\6
${}\{{}$\1\6
\&{if} (\\{closed})\1\5
...
\M{128}Here we impersonate the main dialog loop.
...
\M{129}Feeding various animals leads to various quips. Feeding a dwarf
...
\M{130}Locking and unlocking involves several interesting special cases.
...
\M{131}\B\X131:Open/close grate/chain\X${}\E{}$\6
\&{if} ${}(\\{obj}\E\.{CHAIN}){}$\1\5
...
\M{132}\B\X132:Open/close chain\X${}\E{}$\6
${}\{{}$\1\6
\&{if} ${}(\\{verb}\E\.{OPEN}){}$\1\5
...
\M{134}The clam/oyster is extremely heavy to carry, although not as heavy as
the
gold.
...
\M{135}You get little satisfaction from asking us to read, unless you hold
...
\M{136}OK, that just about does it. We're left with only one more ``action
verb'' to
...
\M{137}\B\X7:Global variables\X${}\mathrel+\E{}$\6
\&{char} ${}{*}\\{incantation}[\,]\K\{\.{"fee"},\39\.{"fie"},\39\.{"foe"},\39%
\.{"foo"},\39\.{"fum"}\};{}$\6
\&{int} \\{foobar};\C{ current incantation progress }\par
\fi

\M{138}Just after every command you give, we make the \PB{\\{foobar}} counter
negative if
...
\M{139}If you get all the way through, we zip the eggs back to the Giant Room,
...
\N{1}{140}Motions. A major cycle comes to an end when a motion verb \PB{%
\\{mot}} has
...
\M{141}When looking around, we pretend that it wasn't dark (though it may {\it
...
\M{142}\B\X7:Global variables\X${}\mathrel+\E{}$\6
\&{int} \\{look\_count};\C{ how many times you've asked us to look }\par
\fi

\M{143}If you ask us to go back, we look for a motion that goes from \PB{%
\\{loc}}
...
\M{144}\B\X22:Additional local registers\X${}\mathrel+\E{}$\6
\&{register} \&{location} \|l${},{}$ \\{ll};\par
\fi

\M{145}\B\X145:Apologize for inability to backtrack\X${}\E{}$\6
${}\{{}$\1\6
...
\M{146}Now we are ready to interpret the instructions in the travel table.
...
\M{147}\B\X147:If the condition of instruction \PB{\|q} isn't satisfied,
advance \PB{\|q}\X${}\E{}$\6
\&{while} (\T{1})\5
${}\{{}$\1\6
${}\|j\K\|q\MG\\{cond};{}$\6
...
\M{148}Here we look at \PB{\\{verb}} just in case you asked us to `\.{find} %
\.{gully}'
...
\M{149}Only the emerald can be toted through the plover-alcove passage ---
not even the lamp.
...
\M{150}The \PB{\\{pdrop}} command applies only when you're carrying the
emerald. We make
...
\M{151}Troll bridge crossing is treated as a special motion so that dwarves
...
\M{152}\B\X152:Block the troll bridge and stay put\X${}\E{}$\6
${}\{{}$\1\6
...
\M{153}Obstacles might still arise after the choice of \PB{\\{newloc}} has been
made.
...
\N{1}{154}Random numbers. You won't realize it until you have played the game
...
\M{158}We kick the random number generator often, just to add variety to the
chase.
...
\N{1}{159}Dwarf stuff. We've said a lot of vague stuff about dwarves; now is
the time
...
\M{160}The following subroutine is often useful.
...
\M{161}Just after you've moved to a new \PB{\\{loc}}, we move the other guys.
...
\M{162}When level 2 is reached, we silently kill 0, 1, or 2 of the dwarves.
...
\M{163}It turns out that the only way you can get rid of a dwarf is to
...
\M{164}Now things are in full swing. Dead dwarves don't do much of anything,
...
\M{165}\B\X7:Global variables\X${}\mathrel+\E{}$\6
...
\M{166}Random-moving dwarves think \PB{\\{scan1}}, \PB{\\{scan2}}, and \PB{%
\\{scan3}} are three different
...
\M{167}A global variable \PB{\\{knife\_loc}} is used to remember where dwarves
have
...
\M{168}\B\X7:Global variables\X${}\mathrel+\E{}$\6
\&{int} \\{knife\_loc};\C{ place where knife was mentioned, or $-1$ }\par
\fi

\M{169}\B\X85:Make special adjustments before looking at new input\X${}%
\mathrel+\E{}$\6
\&{if} ${}(\\{knife\_loc}>\\{limbo}\W\\{knife\_loc}\I\\{loc}){}$\1\5
${}\\{knife\_loc}\K\\{limbo}{}$;\2\par
\fi

\M{170}We actually know the results of the attack already; this is where
...
\M{171}\B\X7:Global variables\X${}\mathrel+\E{}$\6
\&{char} ${}{*}\\{attack\_msg}[\,]\K\{\.{"it\ misses"},\39\.{"it\ gets\ you"},%
\3{-1}\39\.{"none\ of\ them\ hit\ yo}\)\.{u"},\39\.{"one\ of\ them\ gets\ yo}\)%
\.{u"}\}{}$;\par
\fi

\M{172}The pirate leaves you alone once you have found the chest. Otherwise he
...
\M{173}The pirate isn't secretive about the fact that his chest is somewhere
...
\M{174}\B\X174:Snatch all treasures that are snatchable here\X${}\E{}$\6
...
\M{175}The window rooms are slightly lighted, but you don't spot the pirate
there
...
\M{176}One more loose end related to dwarfs needs to be addressed here.
...
\N{1}{177}Closing the cave. You get to wander around until you've located all
...
\M{178}Location Y2 is virtually outside the cave, so \PB{\\{clock1}} doesn't
tick there.
...
\M{179}At the time of first warning, we lock the grate, destroy the crystal
bridge,
...
\M{180}If you try to get out while the cave is closing, we assume that you
panic;
...
\M{181}Finally, after \PB{\\{clock2}} hits zero, we transport you to the final
puzzle,
...
\M{182}Once the cave has closed, we look for objects being toted with \PB{$%
\\{prop}<\T{0}$};
...
\N{1}{183}Death and resurrection. Only the most persistent adventurers get to
...
\M{184}On every turn, we check to see if you are in trouble lampwise.
...
\M{185}\B\X7:Global variables\X${}\mathrel+\E{}$\6
\&{boolean} \\{warned};\C{ have you been warned about the low power supply? }%
\par
\fi

\M{186}The batteries hold a pretty hefty charge.
...
\M{187}\B\X187:Extinguish the lamp\X${}\E{}$\6
${}\{{}$\1\6
${}\\{limit}\K{-}\T{1}{}$;\5
${}\\{prop}[\.{LAMP}]\K\T{0};{}$\6
\&{if} (\\{here}(\.{LAMP}))\1\5
...
\M{188}The easiest way to get killed is to fall into a pit in pitch darkness.
...
\M{190}\B\X7:Global variables\X${}\mathrel+\E{}$\6
...
\M{192}Oh dear, you've disturbed the dwarves.
...
\N{1}{193}Scoring. Here is the scoring algorithm we use:
...
\M{194}The hints are table driven, using several arrays:
...
\M{195}\B\X195:Check if a hint applies, and give it if requested\X${}\E{}$\6
...
\M{197}Here's a subroutine that computes the current score.
...
\M{198}The worst possible score is $-3$.
...
\M{199}\B\X7:Global variables\X${}\mathrel+\E{}$\6
...
\N{1}{200}Launching the program. The program is now complete; all we must
...
\N{1}{201}Index. A large cloud of green smoke appears in front of you. It
clears away
