...
\N[2 gdance.w]{1}{1}Generalized exact cover.
...
\T{100}\C{ at most this many rows in a solution }\par
\B\4\D$\\{max\_degree}$ \5
...
\M[65 gdance.w]{2}\B\X2:Global variables\X${}\E{}$\6
\&{int} \\{verbose};\C{ $>0$ to show solutions, $>1$ to show partial ones too }%
\6
\&{int} \\{count}${}\K\T{0}{}$;\C{ number of solutions found so far }\6
\&{unsigned} \&{int} \\{hupdates}${},{}$ \\{updates};\C{ number of times we
deleted a list element }\6
\&{unsigned} \&{int} \\{hpurifs}${},{}$ \\{purifs};\C{ number of times we
purified a list element }\6
...
\N[77 gdance.w]{1}{3}Data structures.
...
\M[101 gdance.w]{4}Each \&{column} struct contains five fields:
...
\M[122 gdance.w]{5}One \PB{\&{column}} struct is called the root. It serves as
the head of the
...
\M[128 gdance.w]{6}A row is identified not by name but by the names of the
columns it contains.
...
\N[149 gdance.w]{1}{7}Inputting the matrix.
...
\M[156 gdance.w]{8}\B\D$\\{buf\_size}$ \5
...
\M[165 gdance.w]{9}\B\D$\\{panic}(\|m)$ \6
${}\{{}$\5
...
\M[198 gdance.w]{10}\B\X10:Local variables\X${}\E{}$\6
...
\M[204 gdance.w]{11}\B\X11:Read the rows\X${}\E{}$\6
...
\N[236 gdance.w]{1}{12}Backtracking.
...
\M[308 gdance.w]{16}Uncovering is done in precisely the reverse order. The
pointers thereby
...
\M[327 gdance.w]{17}\B\X17:Cover all other columns of \PB{\\{cur\_node}}\X${}%
\E{}$\6
...
\M[332 gdance.w]{18}We included \PB{\\{left}} links, thereby making the rows
doubly linked, so
...
\M[342 gdance.w]{19}\B\X19:Set \PB{\\{best\_col}} to the best column for
branching\X${}\E{}$\6
...
\M[362 gdance.w]{20}\B\X10:Local variables\X${}\mathrel+\E{}$\6
\&{register} \&{int} \\{minlen};\6
\&{register} \&{int} \|j${},{}$ \|k${},{}$ \|x;\par
\fi

\M[366 gdance.w]{21}\B\X21:Record solution and \PB{\&{goto} \\{recover}}\X${}%
\E{}$\6
${}\{{}$\1\6
${}\\{count}\PP;{}$\6
\&{if} (\\{verbose})\5
...
\M[379 gdance.w]{22}\B\X22:Print column lengths, to make sure everything has
been restored\X${}\E{}$\6
${}\{{}$\1\6
...
\M[387 gdance.w]{23}\B\X23:Print a profile of the search tree\X${}\E{}$\6
${}\{{}$\1\6
...
\N[403 gdance.w]{1}{24}Color barriers. Finally, here's the new material related
to coloring.
...
\M[414 gdance.w]{25}When we choose a row that specifies colors in one or more
columns, we
...
\M[442 gdance.w]{26}Just as \PB{\\{purify}} is analogous to \PB{\\{cover}}, the
...
\M[463 gdance.w]{27}\B\X27:Print the color of node \PB{\|q}\X${}\E{}$\6
$\\{printf}(\.{":\%c"},\39\|q\MG\\{color}>\T{0}\?\|q\MG\\{color}:\|q\MG\\{col}%
\MG\\{head}.\\{color}){}$;\par
\U6.\fi

\N[466 gdance.w]{1}{28}Help for debugging. Here's a subroutine for when I'm
