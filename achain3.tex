...
\N[2 achain3.w]{1}{1}Intro. This program is a sequel to {\mc ACHAIN2}, which
you should
...
\M[71 achain3.w]{2}\B\X2:Process the command line\X${}\E{}$\6
\&{if} ${}(\\{argc}\I\T{3}){}$\5
${}\{{}$\1\6
...
\M[77 achain3.w]{3}\B\X3:Output the value of $l_k(n)$\X${}\E{}$\6
\&{if} ${}(\R\\{outfile}[\\{kk}]){}$\5
${}\{{}$\1\6
...
\M[89 achain3.w]{4}Note that the input file for $l_1(n)$ starts with $n=2$, not
$n=1$ as
...
\M[105 achain3.w]{5}\B\X5:Determine $\lfloor\lg n\rfloor$ and the binary upper
bound\X${}\E{}$\6
...
\N[110 achain3.w]{1}{6}The interesting part.
...
\M[148 achain3.w]{7}The only change to this algorithm for {\mc ACHAIN2} occurs
when we happen
...
\M[203 achain3.w]{8}\B\X8:Put local variables on the stack and update
outdegrees\X${}\E{}$\6
...
\M[211 achain3.w]{9}\B\X9:Restore local variables from the stack and downdate
outdegrees\X${}\E{}$\6
...
\M[219 achain3.w]{10}After the test in this step is passed, we'll have \PB{$%
\\{ubq}>\\{ubq}$} and \PB{$\\{lbp}>\\{lbq}$}.
...
\M[239 achain3.w]{11}The undoing mechanism is very simple: When changing \PB{%
\|a[\|j]}, we
...
\M[253 achain3.w]{12}At this point we know that $a[ubq]\le p\le b[ubq]$.
...
\M[284 achain3.w]{13}If, say, we've just set \PB{$\|a[\T{8}]\K\|b[\T{8}]\K%
...
\M[308 achain3.w]{14}At this point we had better not assume that $a[ubq]\le q%
\le b[ubq]$,
...
\M[343 achain3.w]{15}\B\X15:Make forced moves if \PB{\|q} has a special form%
\X${}\E{}$\6
...
\M[351 achain3.w]{16}The bottom line: Alas, this method turns out to be by far
the
