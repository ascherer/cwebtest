...
\N[7 sat12.w]{1}{1}Intro. This program is part of a series of ``SAT-solvers''
that I'm putting
...
\M[54 sat12.w]{2}One of the most important jobs of a preprocessor is to reduce
the
...
\M[104 sat12.w]{3}So here's the structure of the program. (Skip ahead if you
are
...
\M[155 sat12.w]{4}\B\D$\\{show\_basics}$ \5
...
\M[184 sat12.w]{5}On the command line one can specify nondefault values for any
of the
...
\N[249 sat12.w]{1}{6}The I/O wrapper. The following routines read the input and
absorb it into
...
\M[286 sat12.w]{7}Each clause in the temporary tables is represented by a
sequence of
...
\M[301 sat12.w]{8}\B\X4:Global variables\X${}\mathrel+\E{}$\6
...
\M[316 sat12.w]{9}\B\X9:Initialize everything\X${}\E{}$\6
...
\M[332 sat12.w]{10}The hash address of each variable name has $h$ bits, where
$h$ is the
...
\M[382 sat12.w]{11}\B\X11:Input the clause in \PB{\\{buf}}\X${}\E{}$\6
\&{for} ${}(\|j\K\|k\K\T{0};{}$  ; \,)\5
${}\{{}$\1\6
\&{while} ${}(\\{buf}[\|j]\E\.{'\ '}){}$\1\5
...
\M[403 sat12.w]{12}We need a hack to insert the bit codes 1 and/or 2 into a
pointer value.
...
\M[425 sat12.w]{13}\B\X13:Install a new \PB{\&{vchunk}}\X${}\E{}$\6
${}\{{}$\1\6
...
\M[438 sat12.w]{14}\B\X14:Install a new \PB{\&{chunk}}\X${}\E{}$\6
${}\{{}$\1\6
...
\M[451 sat12.w]{15}The hash code is computed via ``universal hashing,'' using
the following
...
\M[458 sat12.w]{16}\B\X16:Put the variable name beginning at \PB{\\{buf}[\|j]}
...
\M[474 sat12.w]{17}\B\X17:Find \PB{$\\{cur\_tmp\_var}\MG\\{name}$} in the hash
table at \PB{\|p}\X${}\E{}$\6
...
\M[484 sat12.w]{18}The most interesting aspect of the input phase is probably
the ``unwinding''
...
\M[493 sat12.w]{19}An input line that begins with `\.{\~\ }' is silently
...
\M[507 sat12.w]{20}\B\X20:Move \PB{\\{cur\_cell}} backward to the previous cell%
\X${}\E{}$\6
...
\M[516 sat12.w]{21}\B\X21:Move \PB{\\{cur\_tmp\_var}} backward to the previous
...
\M[525 sat12.w]{22}\B\X22:Report the successful completion of the input phase%
\X${}\E{}$\6
...
\N[530 sat12.w]{1}{23}SAT preprocessing. This program applies transformations
that
...
\M[568 sat12.w]{24}I haven't spent much time trying to design data structures
that
...
\M[611 sat12.w]{25}The main \PB{\\{mem}} array contains \&{cell} structs, each
occupying three
...
\M[658 sat12.w]{26}Here's a way to display a cell symbolically when debugging
with
...
\M[675 sat12.w]{27}The \PB{\\{vmem}} array contains global information about
...
\M[718 sat12.w]{28}Three octabytes doesn't seem quite enough for the data
associated with
...
\M[726 sat12.w]{29}Similarly, each clause needs more elbow room.
...
\M[741 sat12.w]{30}Here's a subroutine that prints clause number \PB{\|c}.
...
\M[765 sat12.w]{31}Another subroutine shows all the clauses that are currently
in memory.
...
\M[774 sat12.w]{32}With a similar subroutine we can print out all of the
...
\M[798 sat12.w]{33}Speaking of debugging, here's a routine to check if the
links in \PB{\\{mem}}
have gone awry.
...
\M[820 sat12.w]{34}\B\X34:Verify the cells for literal \PB{\|l}\X${}\E{}$\6
${}\{{}$\1\6
...
\M[845 sat12.w]{35}The literals of a clause must appear in increasing order.
...
\M[881 sat12.w]{36}Unused cells of \PB{\\{mem}} either lie above \PB{%
...
\M[894 sat12.w]{37}Of course we need the usual memory allocation routine, to
deliver
...
\M[917 sat12.w]{38}Conversely, we need quick ways to recycle cells that have
done their duty.
...
\M[932 sat12.w]{39}\B\X4:Global variables\X${}\mathrel+\E{}$\6
...
\N[946 sat12.w]{1}{40}Initializing the real data structures.
...
\M[959 sat12.w]{41}There seems to be no good way to predict how many cells
we'll need, because
...
\M[985 sat12.w]{42}\B\X42:Copy all the temporary cells to the \PB{\\{mem}}
...
\M[996 sat12.w]{43}The basic idea is to ``unwind'' the steps that we went
through while
...
\M[1013 sat12.w]{44}\B\X44:Copy all the temporary variable nodes to the \PB{%
...
\M[1020 sat12.w]{45}We should now have unwound all the temporary data chunks
back to their
beginnings.
...
\M[1031 sat12.w]{46}\B\X46:Finish building the cell data structures\X${}\E{}$\6
\&{for} ${}(\|l\K\T{2};{}$ \\{is\_lit}(\|l); ${}\|l\PP){}$\1\5
...
\M[1037 sat12.w]{47}Since we process the literal lists in order, each clause is
...
\M[1065 sat12.w]{48}I'm using two hash bits here, because experiments showed
that this
...
\M[1083 sat12.w]{49}\B\X49:Set the \PB{\\{right}} links for \PB{\|c}, and its
...
\M[1104 sat12.w]{50}Here we assume that \PB{\\{thevar}(\|w)} hasn't already
been eliminated.
...
\M[1126 sat12.w]{51}The logic in this step is similar to the previous one,
...
\M[1148 sat12.w]{52}\B\X52:Allocate the subsidiary arrays\X${}\E{}$\6
...
\N[1163 sat12.w]{1}{53}Clearing the to-do stack.
...
\M[1188 sat12.w]{54}\B\X54:Delete \PB{\\{bar}(\|l)} from all clauses\X${}\E{}$\6
...
\M[1206 sat12.w]{55}\B\X55:Recompute \PB{\\{clssig}(\|c)}\X${}\E{}$\6
${}\{{}$\1\6
...
\M[1215 sat12.w]{56}\B\X56:Delete all clauses that contain \PB{\|l}\X${}\E{}$\6
...
\N[1236 sat12.w]{1}{57}Subsumption testing. Our data structures make it fairly
...
\M[1276 sat12.w]{58}Naturally we seek a literal that appears in the fewest
clauses.
...
\M[1285 sat12.w]{59}The algorithm here actually ends up with either \PB{$\|l<%
\\{ll}$} or \PB{$\|l>\\{ll}$}
in all cases.
...
\M[1308 sat12.w]{60}\B\X60:Remove the subsumed clause \PB{\\{cc}}\X${}\E{}$\6
${}\{{}$\1\6
\&{if} ${}(\\{verbose}\AND\\{show\_details}){}$\1\5
...
\N[1329 sat12.w]{1}{61}Strengthening. A similar algorithm can be used to find
clauses
...
\M[1363 sat12.w]{62}\B\X62:If \PB{\|c} is contained in \PB{\\{cc}}, except for~%
\PB{\|u}, make \PB{$\|l\Z\\{ll}$}\X${}\E{}$\6
$\|o,\39\|q\K\|v,\39\\{qq}\K\\{left}(\\{cc});{}$\6
\&{while} (\T{1})\5
${}\{{}$\1\6
${}\\{oo},\39\|l\K\\{mem}[\|q].\\{lit},\39\\{ll}\K\\{mem}[\\{qq}].\\{lit};{}$\6
...
\M[1384 sat12.w]{63}\B\X63:Remove \PB{\\{bar}(\|u)} from \PB{\\{cc}}\X${}\E{}$\6
${}\{{}$\1\6
...
\M[1418 sat12.w]{64}Clause \PB{\\{cc}} shouldn't become empty at this point.
...
\N[1433 sat12.w]{1}{65}Clearing the strengthened stack.
...
\N[1459 sat12.w]{1}{66}Variable elimination. The satisfiability problem is
essentially the
...
\M[1505 sat12.w]{67}\B\X67:Move \PB{\|v} left\X${}\E{}$\6
${}\{{}$\1\6
${}\|o,\39\|v\K\\{left}(\|v);{}$\6
\&{if} (\\{is\_cls}(\|v))\1\5
${}\|u\K\T{0};{}$\2\6
...
\M[1512 sat12.w]{68}\B\X68:Move \PB{\\{vv}} left\X${}\E{}$\6
${}\{{}$\1\6
${}\|o,\39\\{vv}\K\\{left}(\\{vv});{}$\6
\&{if} (\\{is\_cls}(\\{vv}))\1\5
${}\\{uu}\K\T{0};{}$\2\6
...
\M[1519 sat12.w]{69}\B\X69:Move both \PB{\|v} and \PB{\\{vv}} left\X${}\E{}$\6
${}\{{}$\1\6
...
\M[1525 sat12.w]{70}\B\X70:Copy \PB{\|u} and move \PB{\|v} left\X${}\E{}$\6
${}\{{}$\1\6
...
\M[1532 sat12.w]{71}\B\X71:Copy \PB{\\{uu}} and move \PB{\\{vv}} left\X${}\E{}$%
\6
${}\{{}$\1\6
${}\|q\K\|p,\39\|p\K\\{get\_cell}(\,);{}$\6
...
\M[1539 sat12.w]{72}\B\X72:Copy \PB{\|u} and move both \PB{\|v} and \PB{\\{vv}}
left\X${}\E{}$\6
${}\{{}$\1\6
${}\|q\K\|p,\39\|p\K\\{get\_cell}(\,);{}$\6
${}\\{oo},\39\\{left}(\|q)\K\|p,\39\\{mem}[\|p].\\{lit}\K\|u;{}$\6
...
\M[1546 sat12.w]{73}\B\X73:Return a tautology\X${}\E{}$\6
${}\{{}$\1\6
\&{if} ${}(\|p\I\T{1}){}$\1\5
...
\M[1553 sat12.w]{74}E\'en and Biere, in their paper about preprocessing cited
above,
...
\M[1623 sat12.w]{75}\B\X75:Stamp all literals that appear with \PB{\|l} in
...
\M[1633 sat12.w]{76}\B\X76:If the complements of all other literals in \PB{\|c}
are stamped, set \PB{$\\{beta0}\K\|c$} and \PB{\&{break}}\X${}\E{}$\6
${}\{{}$\1\6
...
\M[1645 sat12.w]{77}\B\X77:Stamp the literals of clause \PB{\\{beta0}}\X${}%
\E{}$\6
...
\M[1652 sat12.w]{78}Now comes the main loop where we test whether the
elimination
...
\M[1701 sat12.w]{79}\B\X79:Decide whether \PB{\|c} belongs to $\alpha^{(0)}$ or
$\alpha^{(1)}$\X${}\E{}$\6
\&{if} ${}(\\{beta0}\E\T{0}){}$\1\5
${}\\{alpha0}\K\T{1};{}$\2\6
\&{else}\5
...
\M[1713 sat12.w]{80}Too bad: We found more resolvents than the clauses they
would replace.
...
\M[1724 sat12.w]{81}The \PB{\\{stamp}} won't overflow because I'm not going to
increase it
...
\M[1737 sat12.w]{82}\B\X52:Allocate the subsidiary arrays\X${}\mathrel+\E{}$\6
\&{if} ${}(\\{buckets}<\T{2}){}$\1\5
${}\\{buckets}\K\T{2};{}$\2\6
...
\M[1746 sat12.w]{83}\B\X83:Try to eliminate variables\X${}\E{}$\6
...
\M[1759 sat12.w]{84}\B\X84:Place candidates for elimination into buckets\X${}%
\E{}$\6
...
\M[1775 sat12.w]{85}\B\X85:Eliminate variable \PB{\|x}, replacing its clauses
...
\M[1796 sat12.w]{86}\B\X86:Update the \.{erp} file for the elimination of \PB{%
\|x}\X${}\E{}$\6
\&{if} (\\{beta0})\5
${}\{{}$\1\6
...
\M[1814 sat12.w]{87}We can't remove the old cells until {\it after\/}
...
\M[1827 sat12.w]{88}Every literal that appears in a new resolvent will be
touched
...
\M[1847 sat12.w]{89}A new resolvent \PB{\\{last\_new}} is waiting to be
...
\M[1863 sat12.w]{90}\B\X90:Install \PB{\\{last\_new}} into position \PB{\|c}%
\X${}\E{}$\6
...
\N[1884 sat12.w]{1}{91}The d\'enouement. ({\it d\'enouement}, n.:\enspace The
final resolution
...
\M[1905 sat12.w]{92}At the beginning we might as well consider every clause to
be
...
\M[1915 sat12.w]{93}Clauses that have been strengthened have also been fully
exploited at
...
\M[1955 sat12.w]{94}\B\X94:Maybe try to strengthen with \PB{\|c}\X${}\E{}$\6
${}\{{}$\1\6
...
\M[1968 sat12.w]{95}\B\X95:Reject \PB{\|u} unless it fills special conditions%
\X${}\E{}$\6
${}\{{}$\1\6
...
\M[1975 sat12.w]{96}\B\X96:Update \PB{\\{newsize}} info for \PB{\|l}'s clauses%
\X${}\E{}$\6
${}\{{}$\1\6
\&{for} ${}(\|o,\39\|p\K\\{down}(\|l);{}$ ${}\R\\{is\_lit}(\|p);{}$ \|o${},\39%
\|p\K\\{down}(\|p)){}$\5
...
\M[1987 sat12.w]{97}\B\X97:Output the simplified clauses\X${}\E{}$\6
\&{for} ${}(\|c\K\\{lit\_head\_top};{}$ \\{is\_cls}(\|c); ${}\|c\PP){}$\1\6
\&{if} ${}(\|o,\39\\{size}(\|c)){}$\5
${}\{{}$\1\6
\&{for} ${}(\|o,\39\|p\K\\{right}(\|c);{}$ ${}\R\\{is\_cls}(\|p);{}$ \|o${},\39%
...
\M[2009 sat12.w]{98}\B\X26:Subroutines\X${}\mathrel+\E{}$\6
