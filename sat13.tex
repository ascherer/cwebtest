...
\N[8 sat13.w]{1}{1}Intro. This program is part of a family of ``SAT-solvers''
that I'm putting
...
\M[69 sat13.w]{2}So here's the structure of the program. (Skip ahead if you are
...
\M[120 sat13.w]{3}On the command line one can specify any or all of the
following options:
\smallskip
\item{$\bullet$}
...
\M[218 sat13.w]{4}\B\X4:Global variables\X${}\E{}$\6
...
\M[270 sat13.w]{5}\B\X5:Respond to a command-line option, setting \PB{\|k}
...
\M[319 sat13.w]{6}\B\X6:If there's a problem, print a message about \.{Usage:}
and \PB{\\{exit}}\X${}\E{}$\6
...
\M[331 sat13.w]{7}\B\X7:Print farewell messages\X${}\E{}$\6
...
\M[402 sat13.w]{10}Each clause in the temporary tables is represented by a
sequence of
...
\M[417 sat13.w]{11}\B\X4:Global variables\X${}\mathrel+\E{}$\6
...
\M[434 sat13.w]{12}\B\X12:Initialize everything\X${}\E{}$\6
...
\M[450 sat13.w]{13}The hash address of each variable name has $h$ bits, where
$h$ is the
...
\M[500 sat13.w]{14}\B\X14:Input the clause in \PB{\\{buf}}\X${}\E{}$\6
\&{for} ${}(\|j\K\|k\K\T{0};{}$  ; \,)\5
${}\{{}$\1\6
\&{while} ${}(\\{buf}[\|j]\E\.{'\ '}){}$\1\5
...
\M[523 sat13.w]{15}We need a hack to insert the bit codes 1 and/or 2 into a
pointer value.
...
\M[545 sat13.w]{16}\B\X16:Install a new \PB{\&{vchunk}}\X${}\E{}$\6
${}\{{}$\1\6
...
\M[558 sat13.w]{17}\B\X17:Install a new \PB{\&{chunk}}\X${}\E{}$\6
${}\{{}$\1\6
...
\M[571 sat13.w]{18}The hash code is computed via ``universal hashing,'' using
the following
...
\M[578 sat13.w]{19}\B\X19:Put the variable name beginning at \PB{\\{buf}[\|j]}
...
\M[594 sat13.w]{20}\B\X20:Find \PB{$\\{cur\_tmp\_var}\MG\\{name}$} in the hash
table at \PB{\|p}\X${}\E{}$\6
...
\M[604 sat13.w]{21}The most interesting aspect of the input phase is probably
the ``unwinding''
...
\M[613 sat13.w]{22}An input line that begins with `\.{\~\ }' is silently
...
\M[627 sat13.w]{23}\B\X23:Move \PB{\\{cur\_cell}} backward to the previous cell%
\X${}\E{}$\6
...
\M[636 sat13.w]{24}\B\X24:Move \PB{\\{cur\_tmp\_var}} backward to the previous
...
\M[645 sat13.w]{25}\B\X25:Report the successful completion of the input phase%
\X${}\E{}$\6
...
\N[650 sat13.w]{1}{26}SAT solving, version 13.
...
\M[741 sat13.w]{27}What data structures support this procedure? We obviously
need to
...
\M[791 sat13.w]{28}Binary clauses $u\lor v$ are represented by putting $v$ into
a list
...
\M[854 sat13.w]{29}The variables are numbered from 1 to \PB{\|n}. The literals
corresponding
...
\M[888 sat13.w]{30}Special data for each literal goes into \PB{\\{lmem}},
...
\M[900 sat13.w]{31}Here is a subroutine that prints the binary implicant data
for
...
\M[914 sat13.w]{32}Similarly, we can print the numbers of all clauses that \PB{%
\|l} is currently
watching.
...
\M[929 sat13.w]{33}And we also sometimes need to see the literals of a given
clause.
...
\M[959 sat13.w]{34}Speaking of debugging, here's a routine to check if the
redundant
...
\M[974 sat13.w]{35}\B\X35:Check all clauses for spurious data\X${}\E{}$\6
...
\M[1028 sat13.w]{36}In really bad cases this routine will get into a loop. I
try to avoid
...
\M[1050 sat13.w]{37}\B\X37:Check the trail\X${}\E{}$\6
...
\M[1095 sat13.w]{38}\B\X38:Check the variables\X${}\E{}$\6
...
\M[1108 sat13.w]{39}The \PB{\\{print\_stats}} subroutine presents a digest of
...
\M[1140 sat13.w]{40}We represent the statistics $\sigma=(x_0+x_1\zeta+x_2%
\zeta^2+\cdots{})/
...
\M[1162 sat13.w]{41}\B\X4:Global variables\X${}\mathrel+\E{}$\6
...
\M[1173 sat13.w]{42}In long runs it's helpful to know how far we've gotten. A
numeric code
...
\M[1221 sat13.w]{43}We might also like to see the complete trail, including
names and reasons.
...
\M[1241 sat13.w]{44}Here's a diagnostic routine that runs through all the
nonbinary,
...
\N[1271 sat13.w]{1}{45}Initializing the real data structures.
...
\M[1287 sat13.w]{46}\B\X46:Allocate \PB{\\{vmem}} and \PB{\\{heap}}\X${}\E{}$\6
...
\M[1302 sat13.w]{47}\B\X47:Allocate the other main arrays\X${}\E{}$\6
\\{free}(\\{buf});\5
...
\M[1325 sat13.w]{48}The \PB{\\{mem}} array will contain $2^k-1<2^{31}$ cells of
four bytes each,
...
\M[1365 sat13.w]{49}Binary data is copied temporarily into cells starting at %
\PB{$\\{min\_learned}+\T{2}$}.
...
\M[1400 sat13.w]{50}The basic idea is to ``unwind'' the steps that we went
through while
...
\M[1415 sat13.w]{51}\B\X51:Do special things for unary and binary clauses\X${}%
\E{}$\6
${}\{{}$\1\6
\&{if} ${}(\|k<\T{2}){}$\1\5
...
\M[1426 sat13.w]{52}We have to watch for degenerate cases: Unit clauses in the
input
...
\M[1438 sat13.w]{53}\B\X53:Reformat the binary implications\X${}\E{}$\6
...
\M[1450 sat13.w]{54}\B\X54:Copy all the temporary variable nodes to the \PB{%
...
\M[1457 sat13.w]{55}We should now have unwound all the temporary data chunks
back to their
beginnings.
...
\M[1470 sat13.w]{56}A few arrays aren't really of ``main'' importance, but we
need to allocate
...
\M[1487 sat13.w]{57}The other arrays can perhaps make use of the memory chunks
that are
...
\N[1498 sat13.w]{1}{58}Forcing. This program spends most of its time adding
literals
...
\M[1544 sat13.w]{59}\B\X4:Global variables\X${}\mathrel+\E{}$\6
\&{uint} \\{lt};\C{ literal on the trail }\6
\&{uint} \\{lat};\C{ its \PB{\\{bimp\_end}} }\6
\&{uint} \\{wa}${},{}$ \\{next\_wa};\C{ a clause in its watch list }\par
\fi

\M[1549 sat13.w]{60}The ``next to inner loop'' of forcing looks for nonbinary
...
\M[1592 sat13.w]{61}The literal \PB{\|l} is known to be permanently false, so
we seize this
...
\M[1605 sat13.w]{62}\B\X62:Swap \PB{\\{wa}} to the watch list of \PB{\|l} and %
\PB{\&{continue}}\X${}\E{}$\6
${}\{{}$\1\6
...
\M[1615 sat13.w]{63}We're looking at clause \PB{\\{wa}}, which is watched by %
\PB{\\{bar}(\\{lt})} and \PB{\\{ll}},
...
\M[1630 sat13.w]{64}A satisfied clause \PB{\\{wa}} can be watched by a false
...
\M[1640 sat13.w]{65}Well, all literals of clause \PB{\\{wa}}, except possibly
the first one,
...
\M[1666 sat13.w]{66}In the case considered here, a conflict has arisen from the
binary clause
...
\M[1683 sat13.w]{67}\B\X67:Deal with a nonbinary conflict\X${}\E{}$\6
${}\{{}$\1\6
\&{if} ${}(\\{verbose}\AND\\{show\_details}){}$\1\5
${}\\{fprintf}(\\{stderr},\39\.{"\ "}\|O\.{"s"}\|O\.{".8s\ from\ "}\|O\.{"d\ \#%
\\n"},\39\\{litname}(\\{ll}),\39\\{wa});{}$\2\6
...
\M[1695 sat13.w]{68}During a ``full run,'' we continue to propagate after
finding a conflict.
...
\M[1717 sat13.w]{69}\B\X69:Record a nonbinary conflict\X${}\E{}$\6
${}\{{}$\1\6
\&{if} ${}(\R\\{conflict\_seen}){}$\5
${}\{{}$\1\6
${}\\{conflict\_seen}\K\T{1};{}$\6
...
\N[1726 sat13.w]{1}{70}Activity scores.
...
\M[1761 sat13.w]{71}The heap contains \PB{\\{hn}} variables, ordered in such a
way that
...
\M[1775 sat13.w]{72}\B\X72:Check the sanity of the heap\X${}\E{}$\6
...
\M[1793 sat13.w]{73}At this point we assume that \PB{$\\{av}\K\\{vmem}[\|v].%
\\{activity}$}.
...
\M[1815 sat13.w]{74}\B\X74:Put \PB{\|v} into the heap\X${}\E{}$\6
${}\{{}$\1\6
...
\M[1823 sat13.w]{75}With probability \PB{\\{rand\_prob}}, we select a variable
...
\M[1858 sat13.w]{76}\B\X76:Set \PB{\|h} to a random integer less than \PB{%
\\{hn}}\X${}\E{}$\6
${}\{{}$\1\6
...
\M[1868 sat13.w]{77}Here we assume that \PB{$\|v\K\\{heap}[\T{0}]$}.
...
\M[1887 sat13.w]{78}At the very beginning, all activity scores are zero.
...
\M[1914 sat13.w]{79}Literals that occur in \PB{\\{polarity\_infile}} must be
...
\M[1951 sat13.w]{80}\B\X4:Global variables\X${}\mathrel+\E{}$\6
...
\M[1957 sat13.w]{81}Learned clauses also have activity scores. They aren't used
as
...
\M[1970 sat13.w]{82}\B\X82:Bump the bumps\X${}\E{}$\6
$\\{var\_bump}\MRL{*{\K}}\\{var\_bump\_factor};{}$\6
${}\\{clause\_bump}\MRL{*{\K}}\\{clause\_bump\_factor}{}$;\par
\Us125\ET133.\fi

\M[1974 sat13.w]{83}When a nonzero activity is rescaled, we are careful to keep
it nonzero
...
\M[1994 sat13.w]{84}\B\D$\\{single\_tiny}$ \5
...
\N[2010 sat13.w]{1}{85}Learning from a conflict.
...
\M[2056 sat13.w]{86}So much for theory; let's proceed to practice. We can use
the \PB{\\{stamp}}
...
\M[2078 sat13.w]{87}\B\X87:Initialize a nonbinary conflict\X${}\E{}$\6
${}\{{}$\1\6
...
\M[2093 sat13.w]{88}Here the conflict is that \PB{\|l} implies \PB{\\{ll}},
where literal \PB{$\|l\K{-}\|c$} is
...
\M[2109 sat13.w]{89}\B\X57:Allocate the auxiliary arrays\X${}\mathrel+\E{}$\6
...
\M[2117 sat13.w]{90}\B\X4:Global variables\X${}\mathrel+\E{}$\6
...
\M[2130 sat13.w]{91}The algorithm that follows will use \PB{\\{curstamp}}, %
\PB{$\\{curstamp}+\T{1}$}, and
\PB{$\\{curstamp}+\T{2}$}.
...
\M[2139 sat13.w]{92}\B\X92:Reduce \PB{\\{xnew}} to zero\X${}\E{}$\6
\&{while} (\\{xnew})\5
${}\{{}$\1\6
\&{while} (\T{1})\5
${}\{{}$\1\6
...
\M[2149 sat13.w]{93}At this point the current conflict clause is represented
implicitly
...
\M[2174 sat13.w]{94}\B\X94:Resolve with binary reason\X${}\E{}$\6
${}\{{}$\1\6
${}\|l\K{-}\|c;{}$\6
\&{if} ${}(\|o,\39\\{vmem}[\\{thevar}(\|l)].\\{stamp}\I\\{curstamp}){}$\1\5
\X95:Stamp \PB{\|l} as part of the conflict clause milieu\X;\2\6
...
\M[2181 sat13.w]{95}\B\X95:Stamp \PB{\|l} as part of the conflict clause milieu%
\X${}\E{}$\6
${}\{{}$\1\6
...
\M[2201 sat13.w]{96}The \PB{\\{stack}} and \PB{\\{conflictdat}} arrays have
...
\M[2228 sat13.w]{97}\B\X4:Global variables\X${}\mathrel+\E{}$\6
...
\M[2235 sat13.w]{98}Here now is the technique of ``on-the-fly subsumption,''
which allows us
...
\N[2285 sat13.w]{1}{99}Simplifying the learned clause.
...
\M[2326 sat13.w]{100}Instead of doing this computation with a recursive
procedure, I want
...
\M[2384 sat13.w]{101}If any of the literals we encounter during that recursive
exploration
...
\M[2401 sat13.w]{102}Sometimes the learned clause turns out to be unnecessarily
long even
...
\M[2430 sat13.w]{103}The following code is used only when \PB{$\\{learned%
\_size}>\T{1}$}. (Learned
...
\M[2457 sat13.w]{104}In early runs of this program, I noticed several times
when the previously
...
\M[2488 sat13.w]{105}The first literal of \PB{\\{prev\_learned}} has no set
value, so it isn't
...
\M[2514 sat13.w]{106}At this point \PB{\|r} is the successor of \PB{\|c} in the
watch list.
...
\M[2525 sat13.w]{107}\B\X107:Store the learned clause \PB{\|c}\X${}\E{}$\6
\&{if} (\\{activ}(\|c))\1\5
\\{confusion}(\.{"bumps"});\2\6
...
\M[2554 sat13.w]{108}\B\X108:Output \PB{\|c} to the file of learned clauses%
\X${}\E{}$\6
${}\{{}$\1\6
...
\N[2563 sat13.w]{1}{109}Recycling unhelpful clauses.
...
\M[2627 sat13.w]{110}The following program computes the scaled range by using
...
\M[2662 sat13.w]{111}\B\X4:Global variables\X${}\mathrel+\E{}$\6
...
\M[2672 sat13.w]{112}Each clause recycling pass is a major event, something
like spring
...
\M[2709 sat13.w]{113}\B\X113:Recycle half of the learned clauses\X${}\E{}$\6
\X114:Compress the database\X;\6
\X122:Recompute all the watch lists\X;\6
${}\\{recycle\_point}\K\T{0}{}$;\par
\U133.\fi

\M[2714 sat13.w]{114}\B\X114:Compress the database\X${}\E{}$\6
...
\M[2741 sat13.w]{115}Clause activity scores are used only to break ties. So
it's natural to
...
\M[2758 sat13.w]{116}\B\X57:Allocate the auxiliary arrays\X${}\mathrel+\E{}$\6
...
\M[2767 sat13.w]{117}Entries of \PB{\\{clause\_heap}} are packed so that they
sort on
...
\M[2784 sat13.w]{118}\B\X118:Establish heap order in the clause heap\X${}\E{}$\6
...
\M[2790 sat13.w]{119}At this point \PB{$\|q\K\|p+\|p+\T{2}$}.
...
\M[2801 sat13.w]{120}We continue to pass over all learned clauses, looking for
those
...
\M[2823 sat13.w]{121}At this point we're operating at root level; that is, %
\PB{$\\{llevel}\K\T{0}$}.
...
\M[2846 sat13.w]{122}\B\X122:Recompute all the watch lists\X${}\E{}$\6
\&{for} ${}(\|l\K\T{2};{}$ ${}\|l\Z\\{max\_lit};{}$ ${}\|l\PP){}$\1\5
...
\M[2858 sat13.w]{123}A technicality for mem counting: We save one memory access
either when
...
\N[2869 sat13.w]{1}{124}Putting it all together.
...
\M[2916 sat13.w]{125}(I should mention somewhere that the updating of \PB{%
\\{agility}} here,
...
\M[2955 sat13.w]{126}\B\X126:Learn a clause of size 1\X${}\E{}$\6
${}\{{}$\1\6
...
\M[2966 sat13.w]{127}\B\X127:Complete the current level, or \PB{\&{goto} %
\\{confl}}\X${}\E{}$\6
...
\M[2980 sat13.w]{128}\B\X128:Backtrack to \PB{\\{jumplev}}\X${}\E{}$\6
${}\{{}$\1\6
...
\M[2996 sat13.w]{129}\B\X129:Print the solution found\X${}\E{}$\6
...
\M[3009 sat13.w]{130}\B\X130:Finish the initialization\X${}\E{}$\6
...
\M[3025 sat13.w]{131}\B\X131:Schedule the next restart\X${}\E{}$\6
...
\M[3032 sat13.w]{132}\B\X132:Schedule the next recycling pass\X${}\E{}$\6
...
\M[3037 sat13.w]{133}After a full cycle has assigned values to all the
variables,
...
\M[3073 sat13.w]{134}Trivial clauses that arise during a full run are ignored,
unless they are
...
\M[3118 sat13.w]{135}\B\X135:Place the literals learned at \PB{\\{minjumplev}}
...
\M[3135 sat13.w]{136}Following the advice of Armin Biere [{\sl Lecture Notes in
Computer
...
\M[3150 sat13.w]{137}Instead of restarting completely, by backing up all the
way to level~0,
...
\M[3184 sat13.w]{138}Well, we didn't solve the problem. Too bad. At least we
can report
...
\M[3219 sat13.w]{139}\B\X139:Debugging fallbacks\X${}\E{}$\6
...
\M[3229 sat13.w]{140}\B\X4:Global variables\X${}\mathrel+\E{}$\6
